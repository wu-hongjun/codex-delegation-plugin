...
- **Marketplace plugin.json is derived.** Never edit
  `marketplace/plugins/cc/.codex-plugin/plugin.json`
...
- Smoke helper `tools/smoke-marketplace.mjs` does not hard-code the
  ...
  `marketplace/plugins/cc/.codex-plugin/plugin.json` at
  startup.
...
`marketplace/plugins/cc/` and the bundled `node_modules/`
introduced by T9.5.
...
1. `npm run build` to refresh the source dist outputs
   ...
   to be current.
2. Copy 24 source-derived files (T2-T5 allowlist) into the
...
- Source ↔ marketplace byte-identity for all 24 derived files
...
Expected output: `check: OK — 24 derived files match source, 64
...
plugin into `<CODEX_HOME>/plugins/cache/cc-plugin-codex-local/cc/<version>/`,
...
- `node <plugin-root>/scripts/cc.mjs setup` must not
...
- `codex plugin marketplace add` succeeds against the local marketplace
  root.
- `codex plugin add "cc@cc-plugin-codex-local"` succeeds.
...
skill-invocation interface, so the thirteen-skill discovery check must be
...
- The other twelve skills must not return `unknown skill` or
...
| `node <plugin-root>/scripts/cc.mjs setup` fails with `ERR_MODULE_NOT_FOUND` | The bundled tree under `marketplace/plugins/cc/node_modules/` is missing or drifted. | Re-run `--write`; verify with `--check`; re-run the smoke. |
...
## Migration from `claude-companion` (Plan 0014)

> **Migration from `claude-companion` (pre-Plan-0014)**: existing installs with the old plugin id must be removed and re-added under the new id:
>
> ```bash
...
# Pre-state (should list cc@cc-plugin-codex-local + the marketplace)
codex plugin list
...
output must not contain `cc@cc-plugin-codex-local` or

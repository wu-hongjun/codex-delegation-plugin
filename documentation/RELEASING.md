...
- **Bundled `node_modules/@cc-plugin-codex/*` use the `<plugin-version>-bundled`
  marker** (e.g., `0.3.4-bundled` while the shipped plugin is `0.3.4`).
...
version is `0.3.4` (bumped in Plan 0022 for the v0.3.4 release).
...
- Tag format: lowercase `v` prefix + semver. The example tag for the
  current shipped version is `v0.3.4`.
...
(`0.3.4` as of Plan 0022), `installed, enabled`. See
[`marketplace/plugins/cc/README.md`](../marketplace/plugins/cc/README.md)

...
- **Bundled `node_modules/@cc-plugin-codex/*` use the `<plugin-version>-bundled`
  marker** (e.g., `0.3.8-bundled` while the shipped plugin is `0.3.8`).
...
version is `0.3.8` (bumped for the v0.3.8 release).
...
- Tag format: lowercase `v` prefix + semver. The example tag for the
  current shipped version is `v0.3.8`.
...
`cc@cc-plugin-codex` with the current version (`0.3.8`),

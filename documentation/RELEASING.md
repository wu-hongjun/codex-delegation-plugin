...
- **Bundled `node_modules/@cc-plugin-codex/*` use the `<plugin-version>-bundled`
  marker** (e.g., `0.3.5-bundled` while the shipped plugin is `0.3.5`).
...
version is `0.3.5` (bumped in Plan 0023 for the v0.3.5 release).
...
- Tag format: lowercase `v` prefix + semver. The example tag for the
  current shipped version is `v0.3.5`.

...
async function acquireAttachLock(shortId) {
...
function isWaiting(sidecar, agentsStatus) {
    if (sidecar) {
    ...
    if (agentsStatus === 'needs_input')
        return true;
    return false;
}
/** Returns true when the sidecar indicates the turn is complete. */
function isTurnComplete(sidecar) {
    if (!sidecar)
        return false;
    if (sidecar.state === 'done')
        return true;
    if (sidecar.tempo === 'idle' &&
...
function hasPromptRegistered(preSend, current, agentsStatus) {
    if (!current) {
        // sidecar unavailable; fall back to agents-json
        return agentsStatus !== undefined && agentsStatus !== 'idle';
    }
    // tempo changed away from idle
    if (current.tempo !== 'idle')
        return true;
    // inFlight tasks/queued increased
    if ((current.inFlight?.tasks ?? 0) > 0)
        return true;
    if ((current.inFlight?.queued ?? 0) > 0)
        return true;
    // output.result changed
    if (current.output?.result !== preSend?.output?.result)
        return true;
    // state changed to waiting (permission stall — already counts as registered)
    if (current.state === 'waiting')
        return true;
    // agents-json fallback
    if (agentsStatus !== undefined && agentsStatus !== 'idle')
        return true;
    return false;
}
// ---------- main export ----------
export async function attachAndSend(session, input, opts) {
    // ── Step 1: Validate ──────────────────────────────────────────────────────
    ...
    const sidecarOpts = { env: opts?.env };
    ...
    const attachWarmupMs = opts?.attachWarmupMs ??
    ...
    const promptRegTimeoutMs = opts?.promptRegisterTimeoutMs ??
    ...
    try {
        ...
        try {
            ...
            throw new DriverError('node-pty could not be loaded. Run `npm rebuild node-pty` or install @homebridge/node-pty-prebuilt-multiarch as a drop-in replacement.', { driverName: DRIVER_NAME, operation: 'send', cause: err });
        }
        ...
        async function handlePermissionIfNeeded() {
            const current = await readSidecar(shortId, sidecarOpts);
            const agentsStatus = await getAgentsStatus();
            if (!isWaiting(current, agentsStatus))
                return;
            if (!opts?.onPermissionRequest) {
                throw new DriverError(`permission required for session \`${shortId}\` but no onPermissionRequest callback was supplied`, { driverName: DRIVER_NAME, operation: 'send' });
            }
        ...
        const finalMessage = finalSidecar?.output?.result ?? undefined;
        // ── Step 12: Detach ───────────────────────────────────────────────────────
        ...
        const detachTimeout = new Promise((res) => {

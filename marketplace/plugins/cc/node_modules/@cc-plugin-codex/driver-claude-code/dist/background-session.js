...
function isUsableIdToken(token) {
    if (NON_ID_TOKENS.has(token.toLowerCase()))
        return false;
    return ID_HEX_SESSION_PATTERN.test(token) || ID_WITH_DIGIT_PATTERN.test(token);
}
function nextIdToken(tokens, from, exclude) {
    for (let j = from; j < tokens.length; j++) {
        ...
        if (KEYWORDS_PRIMARY.includes(t.toLowerCase()))
            continue;
        if (t === exclude)
            continue;
        ...
        if (!NON_ID_TOKENS.has(t.toLowerCase()) && ID_CONTEXTUAL_HEX_PATTERN.test(t))
            return t;
    }
    return undefined;
}
function nextContextToken(tokens, from) {
    for (let j = from; j < tokens.length; j++) {
        const t = tokens[j];
        if (KEYWORDS_PRIMARY.includes(t.toLowerCase()))
            continue;
        if (ID_CONTEXTUAL_PATTERN.test(t))
            return t;
    }
    return undefined;
}
...
export function parseShortId(stdout, stderr, excludeName) {
    ...
    const isExcluded = (t) => excluded !== undefined && t === excluded;
    ...
    if (stdoutLines.length === 1 && isUsableIdToken(stdoutLines[0])) {
        if (!isExcluded(stdoutLines[0]))
            return stdoutLines[0];
        if (nameFallback === undefined)
            nameFallback = stdoutLines[0];
    }
    else if (stdoutLines.length === 1 &&
...
function buildArgv(sessionName, opts) {
    const argv = ['--bg', '--name', sessionName];
    const pushStringArrayFlag = (flag, values) => {
    ...
    const wantsDangerousSkip = opts.dangerouslySkipPermissions === true || opts.permissionMode === 'bypassPermissions';
...
// ---------- startSession ----------
export async function startSession(opts, defaults) {
    // 1. Validate cwd.
    ...
    const sessionName = opts.name && opts.name.trim().length > 0
    ...
    if (result.spawnError) {
        throw new DriverError(`cannot run claude: ${result.spawnError.code ?? result.spawnError.message}`, errCtx({ cause: result.spawnError }));
    }
    // Map timeout to DriverError.
    if (result.timedOut) {
        throw new DriverError(`claude --bg timed out after ${timeoutMs}ms`, errCtx({ stdout: result.stdout, stderr: result.stderr }));
    }
    // Map non-zero exit to DriverError.
    if (result.exitCode !== 0) {
        throw new DriverError(`claude --bg exited ${result.exitCode}`, errCtx({
    ...
    if (!shortId) {
        throw new DriverError('could not parse short ID from claude --bg output', errCtx({ stdout: result.stdout, stderr: result.stderr }));
    }

...
function isMetadata(r) {
    // Explicit meta/metadata type.
    if (r['type'] === 'meta' || r['type'] === 'metadata')
        return true;
...
function classifyRecord(r, _lineNum) {
    ...
    const isNestedMessage = nested !== null &&
    ...
    const isToolCallNoResult = toolCall !== null &&
    ...
    if (isToolUseType || isToolStarted || isToolCallNoResult) {
        ...
        const input = r['input'] ?? (toolCall ? toolCall['input'] : undefined);
        const event = {
            type: 'tool.started',
            tool,
            input,
    ...
    if (isToolResultType || isToolCompleted) {
        const tool = (r['name'] ??
            r['tool'] ??
            (toolCall ? toolCall['name'] : undefined));
    ...
    if (isFileChangeType || isFileField) {
        ...
        const rawOp = (r['operation'] ?? r['op'])?.toLowerCase().trim() ?? '';
        let op;
    ...
    const hasUsageField = usageObj !== null && typeof usageObj === 'object' && !Array.isArray(usageObj);
    const hasTopLevelTokens = 'input_tokens' in r ||
        'output_tokens' in r ||
        'inputTokens' in r ||
        'outputTokens' in r ||
    ...
    if (hasUsageField || hasTopLevelTokens) {
        const src = hasUsageField ? usageObj : r;
        const getNum = (...keys) => {
            for (const k of keys) {
                const v = src[k];
                if (typeof v === 'number')
...
async function searchRootForSessionId(root, sessionId) {
...
export async function discoverTranscriptPath(opts) {
    // 1. Explicit path provided and exists.
    if (opts.transcriptPath) {
        if (await fileExists(opts.transcriptPath))
    ...
    if (opts.sessionId) {
        ...
        const mockHome = opts.env?.['CC_PLUGIN_CODEX_MOCK_CLAUDE_HOME'] ??
...
export async function readTranscriptEvents(opts) {

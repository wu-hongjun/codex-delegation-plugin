...
import { appendEvent, listJobsForWorkspace, readEvents, readJob, syncCompatAliases, updateJob, } from './job-store.js';
 ...
 */
function computeTtlElapsed(job, nowMs, ttlMs) {
...
export function mapStatus(input) {
    ...
    switch (driverValue) {
        ...
        case 'idle': {
            ...
            const sidecarSaysDone = sidecar != null &&
                sidecar.state === 'done' &&
                sidecar.tempo === 'idle' &&
                typeof sidecar.output?.result === 'string' &&
                sidecar.output.result.trim().length > 0;
            const effectiveTurnStatus = sidecarSaysDone &&
...
function resultContextEqual(a, b) {
    if (a === b)
        return true;
    if (!a || !b)
        return false;
    if (a.finalMessagePath !== b.finalMessagePath)
        return false;
    if (a.finalMessagePreview !== b.finalMessagePreview)
        return false;
    if (!deepEqual(a.touchedFiles, b.touchedFiles))
        return false;
    // Compare usageSnapshot too so changing token counts triggers an update.
    return deepEqual(a.usageSnapshot, b.usageSnapshot);
}
function latestTurnIndex(job) {
    return job.turns.length > 0 ? job.turns.length - 1 : null;
}
function deriveShortIdFromSessionId(sessionId) {
    if (!sessionId)
        return undefined;
    const normalized = sessionId.replace(/-/g, '');
    if (!/^[a-fA-F0-9]{8,}$/.test(normalized))
        return undefined;
    return normalized.slice(0, 8);
}
function shouldRepairPseudoShortId(current, derived) {
    return derived !== undefined && current === 'claude';
}
function stringifyWaitingFor(raw) {
...
function waitingForFromSidecar(sidecar) {
    if (sidecar == null)
        return undefined;
    if (sidecar.inFlight?.kinds?.includes('permission'))
        return 'permission';
...
function extractToolTouchedFile(ev) {
    if (ev.type !== 'tool.started')
        return undefined;
    if (!MUTATING_TOOL_NAMES.has(ev.tool.toLowerCase()))
        return undefined;
...
async function writeResultArtifact(jobId, turnIndex, content) {
...
async function preserveLegacySharedResultForPreviousTurn(jobId, patched, currentTurnIndex) {
    ...
    for (let i = currentTurnIndex - 1; i >= 0; i--) {
        ...
        if (result?.finalMessagePath !== sharedPath)
            continue;
        let content;
        try {
...
// ---------- main reconcileJob ----------
export async function reconcileJob(jobId, adapter, options) {
    ...
    const patched = { ...job, session: { ...job.session } };
    ...
    let latestTurnStatusForMapping = 'queued';
    let statusMappingInput = null;
    if (!statusCallFailed && sessionStatus !== null) {
        ...
        const waitingFor = stringifyWaitingFor(sessionStatus.waitingFor) ?? waitingForFromSidecar(sidecar);
        ...
        if (patched.session.provider === 'claude' &&
            ...
            if (patched.session.logsCommand === undefined ||
    ...
    let transcriptEventsForResult = null;
    ...
    if (readArtifacts) {
        ...
        if (adapter.readTranscriptEvents) {
            ...
            try {
                ...
                const touchedFilesOrdered = [];
                const touchedFilesSeen = new Set();
                let lastUsage = null;
                let hasOnlyErrorEvents = true;
                for (const ev of tr.events) {
                    if (ev.type !== 'error')
                        hasOnlyErrorEvents = false;
                    if (ev.type === 'message.completed' && ev.role === 'assistant' && ev.content.length > 0) {
                        finalAssistantMessage = ev;
                    }
                ...
                if (finalAssistantMessage !== null) {
                    turnSnapshotsChanged =
                        (await preserveLegacySharedResultForPreviousTurn(jobId, patched, latestIndexAtStart)) ||
                            turnSnapshotsChanged;
                    const resultPath = await writeResultArtifact(jobId, latestIndexAtStart, finalAssistantMessage.content);
        ...
        const transcriptProducedResult = transcriptSucceeded &&
            transcriptEventsForResult !== null &&
            transcriptEventsForResult.some((ev) => ev.type === 'message.completed' && ev.role === 'assistant');
        ...
        if (statusMappingInput !== null &&
            ...
            nextStatus = mapStatus({
                ...
                sidecar: sidecar?.tempo === 'blocked' || sidecar?.state === 'blocked'
                    ? null
        ...
        const needsLogsFallback = !transcriptAttempted ||
            !transcriptSucceeded ||
            (transcriptEventsForResult !== null &&
                !transcriptEventsForResult.some((ev) => ev.type === 'message.completed' && ev.role === 'assistant'));
        ...
        const sidecarProducedResult = !transcriptProducedResult &&
            typeof sidecar?.output?.result === 'string' &&
            sidecar.output.result.trim().length > 0;
    ...
    const shouldMirrorResultToLatestTurn = newResult !== undefined && (latestTurnHadResultAtStart || resultProducedForLatestTurn);
    ...
    const sessionChanged = patched.session.shortId !== job.session.shortId ||
    ...
    const statusImpliesTurnSync = statusChanged &&
    ...
    if (doAppendEvents) {
        ...
        if (statusChanged) {
            const alreadyEmitted = existingEvents.some((ev) => typeof ev === 'object' &&
                ev !== null &&
                ev['type'] === 'reconcile.status' &&
                ev['previousStatus'] === previousStatus &&
                ev['nextStatus'] === nextStatus);
            if (!alreadyEmitted) {
        ...
        if (resultChanged && newResult !== undefined) {
            const alreadyEmitted = existingEvents.some((ev) => typeof ev === 'object' &&
                ev !== null &&
                ev['type'] === 'reconcile.result' &&
                ev['finalMessagePath'] === newResult.finalMessagePath);
            if (!alreadyEmitted) {
                await appendEvent(jobId, {
        ...
        const emittedWarningMessages = new Set();
        for (const w of warnings) {
            if (emittedWarningMessages.has(w.message))
                continue;
            emittedWarningMessages.add(w.message);
            const alreadyEmitted = existingEvents.some((ev) => typeof ev === 'object' &&
                ev !== null &&
                ev['type'] === 'reconcile.warning' &&
...
export async function reconcileJobsForWorkspace(workspaceRoot, adapter, options) {

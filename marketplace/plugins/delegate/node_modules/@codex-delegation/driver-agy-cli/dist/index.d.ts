import type { Driver, DriverCapabilities, DriverEvent, SendInput, SendOpts, SessionHandle, SessionStatus, StartSessionOpts, TurnHandle, WatchOpts } from '@codex-delegation/runtime';
import type { AgyCliDriverOptions } from './types.js';
export { probeAgyCliDriver } from './probe.js';
export { readAgyState } from './state.js';
export { DRIVER_NAME, DRIVER_VERSION } from './types.js';
export type { AgyCliDriverOptions, AgyRunnerState } from './types.js';
export declare function buildAgyArgs(opts: StartSessionOpts): string[];
export declare function readAgyOutput(path: string | undefined): Promise<string>;
export declare class AgyCliDriver implements Driver {
    private readonly defaults;
    constructor(options?: AgyCliDriverOptions);
    probe(): Promise<DriverCapabilities>;
    startSession(opts: StartSessionOpts): Promise<SessionHandle>;
    watch(target: SessionHandle | TurnHandle, opts?: WatchOpts): AsyncIterable<DriverEvent>;
    status(session: SessionHandle): Promise<SessionStatus>;
    stop(session: SessionHandle): Promise<void>;
    send(_session: SessionHandle, _input: SendInput, _opts?: SendOpts): Promise<TurnHandle>;
    dispose(): Promise<void>;
}
//# sourceMappingURL=index.d.ts.map
...
export class AgyCliDriver {
    ...
    async *watch(target, opts = {}) {
        ...
        for (;;) {
            ...
            if (['completed', 'failed', 'stopped', 'orphaned'].includes(status.value))
                return;
    ...
    async send(session, input, opts = {}) {
        ...
        if (!conversationId) {
            throw new DriverError('agy conversation identity was not captured; start a new job with an agy CLI that supports --conversation and --log-file', { driverName: DRIVER_NAME, operation: 'send' });
        }
        ...
        const startedAt = new Date().toISOString();
        const diagnosticLogPath = previous?.diagnosticLogPath ?? `${session.statePath.slice(0, -'.state.json'.length)}.agy.log`;
        const nextState = {
            schemaVersion: 1,
            driverName: DRIVER_NAME,
            shortId: session.shortId,

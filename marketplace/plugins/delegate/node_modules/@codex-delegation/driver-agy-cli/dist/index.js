...
import { acquireAgySendLock, permissionAnswerKeys, sendAgyControlRequest, sendAgyKeys, } from './control.js';
...
function modeFor(opts) {
    ...
    const requested = [opts.mode, permissionMode, opts.allowEdit ? 'accept-edits' : undefined].filter((value) => value !== undefined);
...
function signalIfRunning(pid, signal) {
...
export function buildAgyResumeArgs(opts) {
    const args = [];
    if (opts.model)
        args.push('--model', opts.model);
    if (opts.agent)
        args.push('--agent', opts.agent);
    const workspaceDirs = Array.from(new Set([opts.cwd, ...(opts.addDirs ?? [])].filter((dir) => typeof dir === 'string' && dir.length > 0)));
    for (const dir of workspaceDirs)
        args.push('--add-dir', dir);
    const mode = modeFor(opts);
    if (mode)
        args.push('--mode', mode);
    if (opts.sandbox || opts.safeMode)
        args.push('--sandbox');
...
async function waitForState(path, timeoutMs) {
...
async function waitForConversationId(path, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    let latest = null;
    for (;;) {
...
function handleStatePath(session) {
...
export async function readAgyOutput(path) {
    if (!path)
        return '';
    try {
...
export class AgyCliDriver {
    ...
    async startSession(opts) {
        ...
        if (observed === null) {
            if (runner.pid && processExists(runner.pid))
                runner.kill('SIGTERM');
        ...
        const identified = await waitForConversationId(statePath, Math.max(this.defaults.timeoutMs ?? 3000, 5000));
        return {
            driverName: DRIVER_NAME,
            shortId,
    ...
    async *watch(target, opts = {}) {
        ...
        for (;;) {
            ...
            if (['needs_input', 'idle', 'completed', 'failed', 'stopped', 'orphaned'].includes(status.value))
                return;
    ...
    async stop(session) {
        ...
        if (state === null || TERMINAL.has(state.status))
            return;
        ...
        while (Date.now() < deadline) {
            const current = await readAgyState(statePath);
            if (current === null || current.status === 'stopped' || TERMINAL.has(current.status))
                return;
            await delay(50);
        }
    ...
    async send(session, input, opts = {}) {
        ...
        if (!conversationId) {
            throw new DriverError('agy conversation identity was not captured; resolve any workspace-trust prompt, then retry', { driverName: DRIVER_NAME, operation: 'send' });
        }
        if (!previous?.controlDir || !processExists(previous.runnerPid)) {
            throw new DriverError('agy interactive supervisor is no longer running; start a new delegated job', {
                driverName: DRIVER_NAME,
                operation: 'send',
            });
        }
        ...
        try {
            ...
            for (;;) {
                if (opts.signal?.aborted) {
                    throw new DriverError('agy follow-up wait was aborted; the interactive session is still running', {
                        driverName: DRIVER_NAME,
                        operation: 'send',
                    });
                }
                ...
                if (current.status === 'failed' || current.status === 'stopped') {
                    const stderr = (await readAgyOutput(session.errorPath)) || (await readAgyOutput(session.resultPath));
                ...
                if (current.status === 'needs_input') {
                    if (!opts.onPermissionRequest) {
                        const error = new DriverError(`Antigravity requires ${current.waitingFor ?? 'interactive input'} but no response callback was supplied`, { driverName: DRIVER_NAME, operation: 'send' });
                ...
                if (Date.now() >= deadline) {
                    throw new DriverError('agy follow-up timed out; the interactive session may still be running', {
                        driverName: DRIVER_NAME,
                        operation: 'send',
                    });
                }

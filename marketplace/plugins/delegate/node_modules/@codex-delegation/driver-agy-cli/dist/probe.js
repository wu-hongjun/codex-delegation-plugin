...
export async function probeAgyCliDriver(options = {}) {
    ...
    const interactiveProbe = interactiveOk
        ? {
    ...
    const targetedResumeOk = helpResult.code === 0 && help.includes('--conversation') && help.includes('--log-file');
    const resumeProbe = targetedResumeOk
        ? {
        ...
        : {
            name: 'agy-targeted-resume',
            status: 'warn',
            detail: 'agy --help does not advertise both --conversation and --log-file; delegation works but exact follow-up is unavailable',
        };
    const modelsResult = binaryOk
        ? await runAgy(['models'], options)
        : { code: null, stdout: '', stderr: '', timedOut: false };
    const authProbe = modelsResult.code === 0 && modelsResult.stdout.trim().length > 0
        ? {
        ...
        : {
            name: 'agy-model-access',
            status: 'warn',
            detail: modelsResult.stderr.trim() ||
    ...
    const pluginResult = binaryOk
        ...
        : { code: null, stdout: '', stderr: '', timedOut: false };
    const controlPluginOk = pluginResult.code === 0 && pluginResult.stdout.includes('codex-delegation-control');
    const controlPluginProbe = controlPluginOk
        ? {
            name: 'agy-control-plugin',
            status: 'ok',
            detail: 'codex-delegation-control lifecycle hooks and native subagent profiles are installed',
        }
        : {
            name: 'agy-control-plugin',
            status: 'warn',
            detail: 'codex-delegation-control is not installed; run $agy-setup for structured lifecycle, transcript discovery, and native subagent profiles',
        };
    let ptyOk = false;
    try {

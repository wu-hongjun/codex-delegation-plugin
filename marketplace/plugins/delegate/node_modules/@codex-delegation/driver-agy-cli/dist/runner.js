...
let hookRefresh = null;
let logPolling = false;
let terminalTextTail = '';
const CONVERSATION_ID = /(?:Created conversation\s+|Print mode:\s+conversation=|--conversation(?:=|\s+))([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/gi;
...
function lastMarker(text) {
    const markers = [
        ...
        { marker: '? for shortcuts', status: 'idle' },
        { marker: 'esc to cancel', status: 'running' },
    ];
    let latest = null;
    for (const marker of markers) {
        const index = text.lastIndexOf(marker.marker);
        if (index < 0 || (latest && latest.index > index))
            continue;
        latest = { index, ...marker };
    }
    return latest;
}
async function observeTerminal(chunk) {
    ...
    if (marker.status === 'idle') {
        ...
        // may settle the turn.
        if (state.hookObserved && state.hookFullyIdle !== true)
            return;
        await publish({
            status: 'idle',
...
async function explicitTranscriptTurnIndex(path) {
    if (!path)
        return undefined;
...
async function captureConversationId() {
    if (!state.diagnosticLogPath)
        return;
    const log = await readFile(state.diagnosticLogPath, 'utf8').catch(() => '');
    let match = null;
    let latest;
    CONVERSATION_ID.lastIndex = 0;
    while ((match = CONVERSATION_ID.exec(log)) !== null)
        latest = match[1]?.toLowerCase();
    if (!latest || latest === state.conversationId)
        return;
    await publish({ conversationId: latest });
}
async function consumeHookState() {
    const hook = JSON.parse(await readFile(hookStatePath, 'utf8'));
    if (!hook.eventId || hook.eventId === state.hookEventId)
        return;
    const patch = {
    ...
    const transcriptTurnIndex = await explicitTranscriptTurnIndex(hook.transcriptPath ?? state.transcriptPath);
...
function refreshHookState() {
    ...
    const pending = consumeHookState()
        ...
        .finally(() => {
        if (hookRefresh === pending)
            hookRefresh = null;
    });
    hookRefresh = pending;
    return pending;
}
async function writeAck(ack) {
    const target = join(acksDir, `${ack.id}.json`);
    const temporary = `${target}.${process.pid}.tmp`;
    await writeFile(temporary, JSON.stringify(ack) + '\n', { mode: 0o600 });
    await rename(temporary, target);
}
async function consumeControlRequest(filename) {
    ...
    try {
        if (!terminal)
            throw new Error('Antigravity terminal is not running');
        if (control.type === 'prompt') {
            if (!control.text?.trim())
                throw new Error('prompt request is empty');
            await publish({
                status: 'running',
            ...
            if (!control.dataBase64)
                throw new Error('keys request is empty');

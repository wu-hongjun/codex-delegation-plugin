...
function filePathFromTool(tool, input) {
    if (!FILE_TOOL_NAMES.has(tool))
        return undefined;
    const args = record(input);
    const candidate = args?.['TargetFile'] ?? args?.['targetFile'] ?? args?.['file_path'] ?? args?.['path'];
    return typeof candidate === 'string' && candidate.length > 0 ? candidate : undefined;
}
function truncate(value, max = 240) {
    return value.length <= max ? value : value.slice(0, max) + '…';
}
export function parseAgyTranscriptJsonl(text) {
    const events = [];
    const warnings = [];
...
export async function readAgyTranscriptEvents(opts) {
...
export function finalAgyAssistantMessage(events) {

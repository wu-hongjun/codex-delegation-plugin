...
import { probeClaudeAgentsJson, probeClaudeAuth, probeClaudeBgFlag, probeClaudeBinary, probeClaudeDaemon, probeClaudeLogs, probeClaudeVersion, probeTranscriptPath, } from '@codex-delegation/runtime';
...
function findProbe(probes, name) {
    return probes.find((p) => p.name === name);
}
function isOk(probes, name) {
    return findProbe(probes, name)?.status === 'ok';
}
function statusOf(probes, name) {
    return findProbe(probes, name)?.status;
}
export async function probeClaudeBackgroundDriver(options = {}) {
    ...
    return {
        ...
        features: {
            start: isOk(probes, 'claude-bg-flag') ||
        ...
        claudeVersion,
        backgroundSessions: isOk(probes, 'claude-bg-flag') ||
            (statusOf(probes, 'claude-bg-flag') === 'warn' && isOk(probes, 'claude-agents-json')),

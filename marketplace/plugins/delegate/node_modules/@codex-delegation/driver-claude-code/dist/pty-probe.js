...
const PTY_SMOKE_ARGS = ['-c', 'echo ok'];
...
async function runPtyBuildProbe() {
    let pty;
    try {
        ...
        return {
            name: 'pty-build',
            status: 'fail',
            detail: `node-pty module load failed: ${err instanceof Error ? err.message : String(err)}. Try \`npm rebuild node-pty\` or switch to @homebridge/node-pty-prebuilt-multiarch.`,
        };
    }
    return await new Promise((resolve) => {
        ...
        try {
            ...
            resolve({
                name: 'pty-build',
                status: 'fail',
                detail: `node-pty spawn threw: ${err instanceof Error ? err.message : String(err)}. Try \`npm rebuild node-pty\` or switch to @homebridge/node-pty-prebuilt-multiarch.`,
            });
        }
    });
}
 ...
 */
export const ptyBuildExtraProbe = {
    name: 'pty-build',
    capabilities: ['followup'],
    run: runPtyBuildProbe,
};
//# sourceMappingURL=pty-probe.js.map
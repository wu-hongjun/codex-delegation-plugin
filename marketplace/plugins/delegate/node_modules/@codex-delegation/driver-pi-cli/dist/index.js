...
export function buildPiResumeArgs(opts, sessionDir) {
    ...
    if (opts.permissionMode === 'plan') {
        throw new DriverError('pi does not expose an enforceable read-only mapping for permission mode "plan"', { driverName: DRIVER_NAME, operation: 'startSession' });
    }
...
export function buildPiArgs(opts, sessionDir) {
...
async function waitState(path, timeout) {
...
async function spawnRunner(requestPath, statePath, request, env) {
...
export class PiCliDriver {
    ...
    async *watch(target, opts = {}) {
        ...
        for (;;) {
            ...
            if (state) {
                ...
                while (count < parsed.events.length)
                    yield parsed.events[count++];
            }
    ...
    async status(session) {
        if (!session.statePath)
            return { value: 'unknown', shortId: session.shortId };
    ...
    async send(session, input, opts = {}) {
        ...
        try {
            ...
            if (!previous || !TERMINAL.has(previous.status) || !sessionId)
                throw new DriverError(!sessionId ? 'pi session identity was not captured' : `pi job is ${previous?.status}`, { driverName: DRIVER_NAME, operation: 'send' });

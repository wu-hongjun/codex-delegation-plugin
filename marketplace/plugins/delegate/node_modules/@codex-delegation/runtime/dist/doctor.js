...
function spawnFailureDetail(cmd, r) {
    if (r.timedOut)
...
// ---------- probes ----------
export async function probeNodeVersion(_opts = {}) {
...
export async function probeCodexVersion(opts = {}) {
...
export async function probeClaudeBinary(opts = {}) {
...
export async function probeClaudeVersion(opts = {}) {
    const r = await runCommand('claude', ['--version'], runOpts(opts));
    if (r.spawnError || r.timedOut || r.exitCode !== 0) {
        return {
            name: 'claude-version',
...
export async function probeClaudeAuth(opts = {}) {
...
export async function probeClaudeBgFlag(opts = {}) {
...
export async function probeClaudeAgentsJson(opts = {}) {
...
export async function probeClaudeLogs(opts = {}) {
...
export async function probeClaudeDaemon(opts = {}) {
...
export async function probeTranscriptPath(opts = {}) {
...
export async function probeCodexPluginTrust(opts = {}) {
    const env = getEnv(opts);
    // Allow tests to point at a sandbox config path. Real path is ~/.codex/config.toml.
    const configPath = env.CODEX_DELEGATION_MOCK_CODEX_TOML ?? join(homedir(), '.codex', 'config.toml');
...
export async function probeDelegationDirWritable(opts = {}) {
...
export async function probeClaudeAttachHelp(opts = {}) {
...
export async function probeClaudeBgNoPrompt(opts = {}) {
    ...
    const meetsFloor = version.major > floor.major ||
...
export async function probeSidecarJobsDir(opts = {}) {
    const env = getEnv(opts);
    const mockHome = env.CODEX_DELEGATION_MOCK_CLAUDE_HOME;
...
function aggregateStatus(probes) {
    if (probes.some((p) => p.status === 'fail'))
        return 'fail';
    if (probes.some((p) => p.status === 'warn'))
        return 'warn';
    return 'ok';
}
function aggregateCapability(probes, capability) {

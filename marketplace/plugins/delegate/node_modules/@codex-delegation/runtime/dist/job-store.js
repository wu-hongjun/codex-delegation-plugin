...
import { CorruptJobRecordError, InvalidJobIdError, JobLockError, JobNotFoundError, } from './errors.js';
import { ensureDelegationDirs, getJobEventsPath, getJobLockPath, getJobRecordPath, getJobsDir, } from './paths.js';
...
async function acquireLock(jobId, operation) {
...
function normalizeClaudeSession(value) {
    const raw = (value ?? {});
    return {
        ...raw,
        provider: 'claude',
        logsCommand: typeof raw.logsCommand === 'string' ? raw.logsCommand : `claude logs ${raw.shortId ?? ''}`,
    };
}
function providerFromDriverName(name) {
    return name === 'agy-cli' ? 'agy' : 'claude';
}
function normalizeSession(value, driverName) {
    const raw = (value ?? {});
    return {
        ...raw,
        provider: typeof raw.provider === 'string' && raw.provider.length > 0
            ? raw.provider
 ...
 * v1 → v2 migration.
 */
function deriveTurnStatusFromJobStatus(status) {
    switch (status) {
        case 'completed':
            return 'completed';
        case 'failed':
 ...
 */
export function syncCompatAliases(record) {
 ...
 */
function migrateJobRecord(raw, path) {
    ...
    if (schemaVersion === 2) {
        ...
        if (!Array.isArray(turns) || turns.length === 0) {
            throw new CorruptJobRecordError(path, new Error('schemaVersion 2 record missing or empty turns array'));
        }
        const driverName = obj['driver']?.name;
        ...
        if (promptDrifted || resultDrifted) {
            ...
            if (expectedResult !== undefined) {
                ...
                void _dropped;
                return { record: syncCompatAliases({ ...withoutResult }), migrated: true };
            }
    ...
    if (schemaVersion === 1 || schemaVersion == null) {
        const v1 = raw;
        const jobStatus = typeof obj['status'] === 'string' ? obj['status'] : 'queued';
        let turnStatus = deriveTurnStatusFromJobStatus(jobStatus);
        ...
        const record = {
            ...
            turns: [turn0],
            ...(v1.result !== undefined ? { result: v1.result } : {}),
            ...(v1.errors !== undefined ? { errors: v1.errors } : {}),
        };
        return { record, migrated: true };
    }
    // 5. Unrecognized schema shape
    throw new CorruptJobRecordError(path, new Error(`unrecognized schemaVersion: ${String(schemaVersion)}`));
}
 ...
 */
async function tryWriteBackMigrated(jobId, _record) {
    let lock = null;
    try {
    ...
    try {
        ...
        let reRaw;
        try {
            reRaw = JSON.parse(await readFile(path, 'utf8'));
        }
        catch {
...
export async function tryReadJob(jobId) {
    validateJobId(jobId);
    const path = getJobRecordPath(jobId);
    let raw;
    try {
    ...
    try {
        ...
        if (err instanceof CorruptJobRecordError)
            throw err;
        throw new CorruptJobRecordError(path, err);
    }
    if (migrated) {
...
export async function appendEvent(jobId, event) {
    validateJobId(jobId);
    await ensureDelegationDirs();
...
export async function listJobs() {
    ...
    for (const entry of entries) {
        ...
        if (entry.endsWith('.tmp'))
            continue; // in-flight atomic write artifact
...
export async function listJobsForWorkspace(workspaceRoot) {

 ...
 *
 * @param {import('@codex-delegation/runtime').ReviewOfContext | undefined} reviewOf
 * @returns {string}
 */
...
function classifyWaiting(waitingFor, opts = {}) {
  ...
  const note =
    category === 'browser_selection'
      ...
      : kind === 'permission'
        ? unattendedRequested
          ? 'Claude Code is waiting on a permission prompt even though unattended bypass was requested. The delegate wrapper cannot safely approve it non-interactively; attach to inspect, or stop and rerun after fixing the underlying Claude Code/tool configuration.'
          : 'Claude Code is waiting on a permission prompt. The delegate wrapper cannot approve it non-interactively; attach to the session or restart a trusted future run with an explicit permission mode.'
...
/**
 * @param {import('@codex-delegation/runtime').DoctorReport} report
 * @param {boolean} json
 * @returns {string}
 */
export function formatSetup(report, json) {
  ...
  const sections = [];
  sections.push(
    `Codex Delegation setup — ${overall}`,
...
/**
 * @param {import('@codex-delegation/runtime').JobRecord[]} jobs
 * @param {boolean} json
...
export function formatStatus(jobs, json, workspaceRoot, opts = {}) {
  ...
  if (opts.singleJob) {
    ...
    if (opts.versionMismatch) {
      lines.push(
        '',
        `Version warning: dispatcher ${opts.versionMismatch.dispatcherVersion} is running while this workspace declares ${opts.versionMismatch.worktreeVersion}. Refresh the installed delegate plugin or use the workspace dispatcher directly.`,
      );
    }
    return lines.join('\n');
  }
...
/**
 * @param {import('@codex-delegation/runtime').JobRecord} job
 * @param {import('@codex-delegation/runtime').TurnHandle | null} turnHandle
 ...
 *
 * @param {{ review: import('./review-parser.mjs').ReviewOutput; job: import('@codex-delegation/runtime').JobRecord; turn: object }} opts
 * @returns {string}
 */
 ...
 *
 * @param {{ review: import('./review-parser.mjs').ReviewOutput; job: import('@codex-delegation/runtime').JobRecord; targetJob: import('@codex-delegation/runtime').JobRecord }} opts
 * @returns {string}
 */

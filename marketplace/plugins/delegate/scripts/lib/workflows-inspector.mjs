...
//   sessions started via 'delegate workflow' (--bg). See Plan 0016 OQ-A artifact
//   (oq-a-workflows-ansi-capture-20260606.txt) for the empirical evidence.
//
//   This module reads from the local codex-delegation-plugin job store at
//   ~/.codex/codex-delegation-plugin/jobs/*.json (resolved via the runtime's
//   getJobsDir helper, which honors CODEX_DELEGATION_HOME for tests). The
...
import { listJobs } from '@codex-delegation/runtime';
...
/**
 * List all codex-delegation-plugin background jobs that were started as workflows.
 *
 ...
 *
 * Reads:
 *   - The local job record at ~/.codex/codex-delegation-plugin/jobs/<jobId>.json
...
export async function inspectWorkflow(jobId) {
  ...
  // Accept either the delegate job id (job_…) OR the Claude session id. The list
  // view shows the 8-char session shortId in its first column and instructs
  // "delegate workflows <sessionId>", so the displayed id must resolve here.

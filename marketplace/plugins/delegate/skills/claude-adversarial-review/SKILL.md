...
2. Resolve `<plugin-root>` as the parent directory of the `skills/` directory
   that contains this file (so `<plugin-root>/scripts/delegate.mjs` is the
   dispatcher). Confirm `<plugin-root>/scripts/delegate.mjs` exists before running.

3. Run the dispatcher:

       node "<plugin-root>/scripts/delegate.mjs" adversarial-review <jobId-or-prefix> [flags]
   ...
   include `--yes` in the same command so the review job stays inside the delegate

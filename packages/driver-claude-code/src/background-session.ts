...
//      for non-bypass modes. Bypass launches use Claude Code's literal
//      --dangerously-skip-permissions flag because that is the unattended
//      boolean surface operators expect.
...
function buildArgv(sessionName: string, opts: StartSessionOpts): string[] {
  ...
  const wantsDangerousSkip =
    opts.dangerouslySkipPermissions === true || opts.permissionMode === 'bypassPermissions';
  if (wantsDangerousSkip) {
    argv.push('--dangerously-skip-permissions');
  } else if (opts.permissionMode) {
    // OQ1: only pass --permission-mode when the caller explicitly set a non-bypass mode.
  ...
  // Delimit the prompt because Claude Code's --add-dir flag is variadic.

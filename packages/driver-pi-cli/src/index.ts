...
import type {
  Driver, DriverCapabilities, DriverEvent, SendInput, SendOpts, SessionHandle,
  SessionStatus, StartSessionOpts, TurnHandle, WatchOpts,
...
export function buildPiResumeArgs(opts: StartSessionOpts, sessionDir: string): string[] {
  ...
  } else if (opts.permissionMode && !['default', 'plan'].includes(opts.permissionMode)) {
...
function statusFrom(state: PiRunnerState): SessionStatus {
  const value: SessionStatus['value'] =
    state.status === 'running' ? 'working' : state.status === 'starting' ? 'starting' : state.status;
  return {
    value, shortId: state.shortId, sessionId: state.sessionId, sessionName: state.sessionName,
    cwd: state.cwd, pid: state.piPid ?? state.runnerPid, startedAt: state.startedAt,
    updatedAt: state.updatedAt, transcriptPath: state.transcriptPath, raw: state,
  };
}
...
async function spawnRunner(requestPath: string, statePath: string, request: PiLaunchRequest): Promise<ChildProcess> {
  ...
  try {
    const runner = spawn(process.execPath, [RUNNER_PATH, requestPath, statePath], {
      cwd: request.cwd, env: request.env, detached: true, shell: false, stdio: 'ignore',
    });
    await new Promise<void>((resolve, reject) => {
      runner.once('spawn', resolve); runner.once('error', reject);
    });
    runner.unref();
    return runner;
  } catch (error) {
...
export class PiCliDriver implements Driver {
  ...
  probe(): Promise<DriverCapabilities> { return probePiCliDriver(this.defaults); }

  async startSession(opts: StartSessionOpts): Promise<SessionHandle> {
    if (!opts.prompt.trim()) throw new DriverError('startSession requires a non-empty prompt',
      { driverName: DRIVER_NAME, operation: 'startSession' });
    const cwd = opts.cwd || this.defaults.cwd;
    if (!cwd) throw new DriverError('startSession requires cwd',
      { driverName: DRIVER_NAME, operation: 'startSession' });
    ...
    const state: PiRunnerState = {
      schemaVersion: 1, driverName: DRIVER_NAME, shortId, sessionName, cwd,
      status: 'starting', runnerPid: 0, startedAt, updatedAt: startedAt,
      resultPath, errorPath, transcriptPath, sessionDir, resumeArgs, turnIndex: 0,
    };
    await Promise.all([writeFile(transcriptPath, '', { mode: 0o600 }),
      writeFile(errorPath, '', { mode: 0o600 }), writeFile(resultPath, '', { mode: 0o600 })]);
    let runner: ChildProcess;
    try {
      runner = await spawnRunner(requestPath, statePath, {
        executable: executable(this.defaults), args: [...resumeArgs, opts.prompt], cwd,
        env: { ...process.env, ...this.defaults.env }, state,
      });
    } catch (error) {
      throw new DriverError('Failed to start the pi supervisor',
        { driverName: DRIVER_NAME, operation: 'startSession', cause: error });
    }
    ...
    return {
      driverName: DRIVER_NAME, shortId, sessionName, cwd, startedAt,
      ...(observed.sessionId ? { sessionId: observed.sessionId } : {}),
      pid: observed.piPid ?? runner.pid, statePath, resultPath, errorPath,
    };
  }

  async *watch(target: SessionHandle | TurnHandle, opts: WatchOpts = {}): AsyncIterable<DriverEvent> {
    const session = 'session' in target ? target.session : target;
    let count = 0; let previous = '';
    for (;;) {
      ...
      if (status.value !== previous) {
        yield { type: 'session.status', status, at: new Date().toISOString() }; previous = status.value;
      }
  ...
  async status(session: SessionHandle): Promise<SessionStatus> {
    ...
    if (!state) return { value: exists(session.pid) ? 'starting' : 'orphaned', shortId: session.shortId };
    return !TERMINAL.has(state.status) && !exists(state.runnerPid)
      ? { ...statusFrom(state), value: 'orphaned' } : statusFrom(state);
  }
  ...
  async stop(session: SessionHandle): Promise<void> {
    ...
    for (const pid of [state.piPid, state.runnerPid]) {
      if (exists(pid)) try { process.kill(pid!, 'SIGTERM'); } catch { /* raced exit */ }
    }
    ...
    for (const pid of [state.piPid, state.runnerPid]) {
      if (exists(pid)) try { process.kill(pid!, 'SIGKILL'); } catch { /* raced exit */ }
    }
    const now = new Date().toISOString();
    await writePiState(session.statePath, { ...state, status: 'stopped', signal: 'SIGKILL',
      updatedAt: now, endedAt: now });
  }

  async send(session: SessionHandle, input: SendInput, opts: SendOpts = {}): Promise<TurnHandle> {
    if (!input.text.trim() || !session.statePath) throw new DriverError('pi follow-up requires text and persisted state',
      { driverName: DRIVER_NAME, operation: 'send' });
    const lock = `${session.statePath}.send.lock`;
    try { await mkdir(lock); }
    catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'EEXIST') throw new DriverError('pi follow-up is already in progress',
        { driverName: DRIVER_NAME, operation: 'send', cause: error });
      throw error;
    }
    const startedAt = new Date().toISOString();
    try {
      ...
      if (!previous || !TERMINAL.has(previous.status) || !sessionId) throw new DriverError(
        !sessionId ? 'pi session identity was not captured' : `pi job is ${previous?.status}`,
        { driverName: DRIVER_NAME, operation: 'send' });
      const turnIndex = previous.turnIndex + 1;
      const transcriptPath = join(dirname(previous.transcriptPath), `${session.shortId}.turn-${turnIndex}.jsonl`);
      await Promise.all([writeFile(transcriptPath, '', { mode: 0o600 }), writeFile(previous.errorPath, '', { mode: 0o600 })]);
      const next: PiRunnerState = { ...previous, status: 'starting', runnerPid: 0, piPid: undefined,
        transcriptPath, turnIndex, startedAt, updatedAt: startedAt, endedAt: undefined,
        exitCode: undefined, signal: undefined, error: undefined };
      ...
      await spawnRunner(requestPath, session.statePath, {
        ...
        cwd: previous.cwd, env: { ...process.env, ...this.defaults.env }, state: next,
      });
      const deadline = Date.now() + (opts.timeoutMs ?? 600_000);
      for (;;) {
        if (opts.signal?.aborted) throw new DriverError('pi follow-up wait aborted; process remains supervised',
          { driverName: DRIVER_NAME, operation: 'send' });
        const current = await readPiState(session.statePath);
        if (!current) throw new DriverError('pi state disappeared during follow-up',
          { driverName: DRIVER_NAME, operation: 'send' });
        ...
        if (current.turnIndex < turnIndex) {
          if (Date.now() >= deadline) throw new DriverError('pi follow-up timed out before supervisor startup',
            { driverName: DRIVER_NAME, operation: 'send' });
          await delay(20);
          continue;
        }
        if (TERMINAL.has(current.status)) {
          ...
          if (current.status !== 'completed') {
            ...
            throw new DriverError(current.error ?? `pi follow-up ${current.status}`,
              { driverName: DRIVER_NAME, operation: 'send', stderr });
          }
          return { driverName: DRIVER_NAME, session: { ...session, sessionId }, startedAt,
            endedAt: current.endedAt, status: 'completed',
            ...(parsed.finalMessage ? { finalMessage: parsed.finalMessage } : {}) };
        }
        if (Date.now() >= deadline) throw new DriverError('pi follow-up timed out; process remains supervised',
          { driverName: DRIVER_NAME, operation: 'send' });
        await delay(100);
      }
    } finally {

...
export interface PiTranscriptWarning { line: number; message: string }
...
export function parsePiTranscriptJsonl(text: string): {
  events: DriverEvent[]; warnings: PiTranscriptWarning[]; sessionId?: string; finalMessage?: string;
} {
  ...
  for (const [index, source] of text.split(/\r?\n/).entries()) {
    ...
    try {
      ...
      warnings.push({ line: index + 1, message: error instanceof Error ? error.message : 'invalid JSON' });
      continue;
    }
    const type = raw['type'];
    ...
    if (type === 'tool_execution_start' && typeof raw['toolName'] === 'string') {
      ...
      events.push({
        type: 'tool.completed', tool: raw['toolName'], ok: raw['isError'] !== true,
        ...(raw['result'] === undefined ? {} : { resultPreview: String(raw['result']).slice(0, 240) }), at, raw,
      });
  ...
  return { events, warnings, ...(sessionId ? { sessionId } : {}), ...(finalMessage ? { finalMessage } : {}) };
}

export async function readPiTranscript(path: string): Promise<ReturnType<typeof parsePiTranscriptJsonl>> {
  try { return parsePiTranscriptJsonl(await readFile(path, 'utf8')); }
  catch (error) {

...
describe('consolidated release checklist (Plan 0006 T11)', () => {
  ...
  // ========================================================================
  // T11-8: Tagging section requires smoke + CI before tag + has v0.3.13 example
  // ========================================================================

  it('Tagging section gates on smoke + CI and shows v0.3.13 example', () => {
    const content = readFileSync(RELEASING_MD, 'utf8');
    assert.match(
      content,
    ...
    assert.ok(
      content.includes('v0.3.13'),
      'Tagging section must include the current example tag `v0.3.13`',
    );
    assert.match(
      content,

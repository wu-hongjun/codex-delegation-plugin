#!/usr/bin/env node
// delegate.mjs — user-facing CLI dispatcher for the codex-delegation-plugin plugin.
//
...
const UPGRADE_TARGETS = Object.freeze({
  public: Object.freeze({
    source: 'public',
    marketplace: 'codex-delegation-plugin',
    plugin: 'delegate@codex-delegation-plugin',
    repoUrl: 'https://github.com/wu-hongjun/codex-delegation-plugin',
    refreshMarketplace: true,
  }),
  local: Object.freeze({
    source: 'local',
    marketplace: 'codex-delegation-plugin-local',
    plugin: 'delegate@codex-delegation-plugin-local',
    repoUrl: null,
...
function findWorkspacePluginVersion(cwd) {
  const candidates = [
    join(cwd, 'packages', 'plugin-delegate', '.codex-plugin', 'plugin.json'),
...
function dispatcherPathForHints() {
  ...
  const currentDispatcher = join(dirname(pluginRoot), 'current', 'scripts', 'delegate.mjs');
...
function discoverClaudeSkills({ cwd = process.cwd(), env = process.env } = {}) {
  const claudeHome = env.CODEX_DELEGATION_MOCK_CLAUDE_HOME || join(homedir(), '.claude');
  const errors = [];
  const roots = [];
  const skills = [];
...
if (command !== 'doctor' && (flags['real'] !== undefined || flags['claude-access'] !== undefined)) {
  ...
  const message =
    badFlag === '--real'
      ? '--real is only supported by delegate doctor as a preflight alias; use --chrome when launching Claude Code.'
      : '--claude-access is only supported by delegate doctor.';
...
function makeSetupExtraProbes() {
  ...
  /** @type {import('@codex-delegation/runtime').DoctorExtraProbe} */
  ...
  const worktreeVersionProbe = {
    name: 'delegate-worktree-version',
    capabilities: [],
    run: async (opts) => {
      ...
      if (mismatch === null) {
        return {
          name: 'delegate-worktree-version',
          status: 'ok',
          detail: `dispatcher version ${PLUGIN_VERSION} matches visible delegate worktree, or no delegate worktree was detected`,
        };
      }
      return {
        name: 'delegate-worktree-version',
        status: 'warn',
        detail: `dispatcher version ${mismatch.dispatcherVersion} differs from workspace plugin version ${mismatch.worktreeVersion}; refresh the installed plugin or run node packages/plugin-delegate/scripts/delegate.mjs for development testing`,
        evidence: mismatch,
      };
    },
  };
...
function formatDoctor(preflight, json) {
  ...
  const lines = [
    `Codex Delegation doctor preflight — ${label}`,
...
async function cmdDoctor(flags, positional, json) {
  if (positional.length > 0) {
    process.stderr.write(
      formatError(new Error('delegate doctor does not take positional arguments'), 'doctor', json) +
        '\n',
    );
    process.exit(2);
  }
...
async function cmdWorkflow(flags, positional, json) {
  ...
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(
        new Error('--allow-edit is not applicable to delegate workflow.'),
        'workflow',
        json,
      ) + '\n',
    );
...
async function cmdGoal(flags, positional, json) {
  ...
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(new Error('--allow-edit is not applicable to delegate goal.'), 'goal', json) +
        '\n',
    );
    process.exit(2);
  }
...
async function cmdFork(flags, positional, json) {
  ...
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(new Error('--allow-edit is not applicable to delegate fork.'), 'fork', json) +
        '\n',
    );
    process.exit(2);
  }
...
async function cmdBatch(flags, positional, json) {
  ...
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(new Error('--allow-edit is not applicable to delegate batch.'), 'batch', json) +
        '\n',
    );
    process.exit(2);
  }
...
async function cmdDeepResearch(flags, positional, json) {
  ...
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(
        new Error('--allow-edit is not applicable to delegate deep-research.'),
        'deep-research',
        json,
      ) + '\n',
    );
...
async function _runDelegateCore(
  ...
  if (!rawPrompt) {
    process.stderr.write(
      formatError(
        new Error(`prompt is required: delegate ${commandName} -- "<prompt>"`),
        commandName,
        json,
      ) + '\n',
    );
...
async function cmdRestart(flags, positional, json) {
  ...
  if (!prefix) {
    process.stderr.write(
      formatError(
        new Error('usage: delegate restart <jobId-or-prefix> [fresh-session-flags] -- "<prompt>"'),
        'restart',
        json,
      ) + '\n',
    );
  ...
  if (!prompt) {
    ...
    process.stderr.write(
      formatError(
        new Error(
          `restart requires a fresh prompt because delegate job records store only prompt metadata. Run: ${restartExample}`,
        ),
        'restart',
        json,
      ) + '\n',
...
async function cmdStatus(flags, positional, json) {
  ...
  if (positional.length > 0) {
    process.stderr.write(
      formatError(
        new Error(
          `delegate status does not take a job id (got "${positional[0]}"). ` +
  ...
  if (jobFlag !== undefined) {
    ...
    if (!prefix) {
      process.stderr.write(
        formatError(new Error('usage: delegate status --job <jobId-or-prefix>'), 'status', json) +
          '\n',
      );
      process.exit(2);
    }
...
async function cmdResult(flags, positional, json) {
  ...
  if (!prefix) {
    process.stderr.write(
      formatError(new Error('usage: delegate result <jobId>'), 'result', json) + '\n',
    );
    process.exit(2);
  }
...
async function cmdWait(flags, positional, json) {
  ...
  if (!prefix) {
    process.stderr.write(
      formatError(new Error('usage: delegate wait <jobId>'), 'wait', json) + '\n',
    );
    process.exit(2);
  }
  if (positional.length > 1) {
    process.stderr.write(
      formatError(
        new Error(`delegate wait takes exactly one job id (got extra argument "${positional[1]}")`),
        'wait',
        json,
      ) + '\n',
    );
...
async function cmdStop(flags, positional, json) {
  ...
  if (!prefix) {
    ...
    process.stderr.write(
      formatError(new Error('usage: delegate stop <jobId>'), 'stop', json) + '\n',
    );
    process.exit(2);
  }
...
async function sendFollowupTurn({
  ...
  // CODEX_DELEGATION_PERMISSION_TIMEOUT_MS environment variable. This is a test seam
  ...
  const rawTimeoutOverride = process.env.CODEX_DELEGATION_PERMISSION_TIMEOUT_MS;
...
async function cmdFollowup(flags, positional, json) {
  ...
  if (!prefix) {
    process.stderr.write(
      formatError(
        new Error('usage: delegate followup <jobId-or-prefix> [flags] -- "<prompt>"'),
        'followup',
        json,
      ) + '\n',
    );
  ...
  if (!prompt) {
    process.stderr.write(
      formatError(
        new Error(`prompt is required: delegate followup <jobId-or-prefix> -- "<prompt>"`),
        'followup',
        json,
      ) + '\n',
    );
...
async function cmdReview(flags, positional, json) {
  ...
  if (!prefix) {
    process.stderr.write(
      formatError(
        new Error('usage: delegate review <jobId-or-prefix> [--all] [--json] [--yes]'),
        'review',
        json,
      ) + '\n',
    );
  ...
  // bypassed entirely under the test seam CODEX_DELEGATION_REVIEW_RECONCILE_DELAY_MS
  // (set to 0 by the mock-driven test env).
  const reviewWaitRaw = process.env.CODEX_DELEGATION_REVIEW_RECONCILE_DELAY_MS;
...
async function cmdAdversarialReview(flags, positional, json) {
  ...
  if (!prefix) {
    process.stderr.write(
      formatError(
        new Error(
          'usage: delegate adversarial-review <jobId-or-prefix> [--all] [--json] [--yes] [--model <model>] [--effort <effort>] [--permission-mode <mode>]',
        ),
        'adversarial-review',
        json,
      ) + '\n',
  ...
  // Env-var override: CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS.
  // Defensive parse: parseInt; NaN or <= 0 → use default.
  //
  // Poll interval default: 2000 ms. Override via CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS
  // (TEST SEAM ONLY — not user-facing).
  const ADVERSARIAL_REVIEW_TIMEOUT_DEFAULT_MS = 1_800_000;
  const rawTimeoutEnv = process.env.CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS;
  ...
  const rawPollEnv = process.env.CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS;
...
async function cmdWorkflows(flags, positional, json) {
  ...
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(
        new Error(
          '--allow-edit is not applicable to delegate workflows; this command is read-only.',
        ),
        'workflows',
        json,
      ) + '\n',
...
async function cmdSkills(flags, positional, json) {
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(
        new Error('--allow-edit is not applicable to delegate skills; this command is read-only.'),
        'skills',
        json,
      ) + '\n',
    );
...
function detectUpgradeTarget(flags) {
  ...
  if (scriptPath.includes('/codex-delegation-plugin-local/')) return UPGRADE_TARGETS.local;
...
function upgradeSourceLabel(target) {
  return target.source === 'local'
    ? 'local marketplace (codex-delegation-plugin-local)'
    : 'public Git marketplace (codex-delegation-plugin)';
}
...
async function cmdUpgrade(flags, positional, json) {
  if (flags['allow-edit'] !== undefined) {
    process.stderr.write(
      formatError(
        new Error(
          '--allow-edit is not applicable to delegate upgrade; it only calls Codex plugin commands.',
        ),
        'upgrade',
        json,
      ) + '\n',
  ...
  if (json) {
    ...
    const lines = ['Codex Delegation plugin refresh complete.', ''];
...
function printUsage(commandName = '') {
  const commandHelp = {
    ...
    restart: [
      ...
      'The original prompt is not replayed because delegate stores prompt metadata, not full prompt text.',
    ...
    status: [
      'Usage: delegate status [--all] [--json] [--compact] [--limit <n>] [--stored-status <state>]',
      '       delegate status --job <jobId-or-prefix> [--all] [--json]',
      '',
    ...
    wait: [
      'Usage: delegate wait <jobId-or-prefix> [--all] [--json] [--compact] [--timeout <duration>] [--interval <duration>]',
      '',
    ...
    stop: [
      'Usage: delegate stop <jobId-or-prefix> [--all] [--json] [--compact]',
      '       delegate stop --all-awaiting-followup [--all] [--json]',
      '       delegate stop --all-needs-input [--all] [--json]',
      '',
    ...
    upgrade: [
      ...
      '',
      'Refreshes or repairs the installed Codex Delegation plugin through the Codex CLI.',

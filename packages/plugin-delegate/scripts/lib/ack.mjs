...
import { getDelegationHome } from '@codex-delegation/runtime';
 ...
 */
function acksDir() {
  return join(getDelegationHome(), 'acks');
}
...
function ackPath(workspaceRoot, provider) {
  const normalizedProvider = String(provider || 'claude')
    ...
    .toLowerCase()
    .replace(/[^a-z0-9_-]/g, '_');
  return join(acksDir(), `${workspaceHash(workspaceRoot)}.${normalizedProvider}.json`);
}

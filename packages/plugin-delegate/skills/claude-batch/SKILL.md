...
You are the Codex skill wrapper for the delegate dispatcher's batch
subcommand.
...
Run:
    ...
    node "<plugin-root>/scripts/delegate.mjs" batch -- "<instruction>"

Forwarded flags go before `--`; everything after `--` is the instruction
verbatim. Example:

    node "<plugin-root>/scripts/delegate.mjs" batch --model claude-opus-4-8 --name migration -- "<instruction>"

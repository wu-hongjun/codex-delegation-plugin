...
Run:
    ...
    node "<plugin-root>/scripts/delegate.mjs" status
...
On macOS, do not rewrite those hints to a bare `delegate ...` shell command; that
...
`"<prompt>"` placeholder because delegate stores prompt metadata, not the full
...
must be resolved with `claude attach <shortId>` in the Claude Code TUI. The delegate
...
When running from a codex-delegation-plugin checkout, status may include a
...
the installed plugin or run `node packages/plugin-delegate/scripts/delegate.mjs`

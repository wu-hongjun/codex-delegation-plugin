...
Run:
    ...
    node "<plugin-root>/scripts/delegate.mjs" workflows [<jobId>]
...
- No args → list all workflow-like sessions visible in the codex-delegation-plugin job store.
...
the codex-delegation-plugin job store and Claude project metadata. No Claude Code
session is spawned.

// Tests for the delegate.mjs dispatcher — T10
//
...
// temp directories for CODEX_DELEGATION_HOME, CODEX_DELEGATION_MOCK_CLAUDE_HOME, and
...
//
// Privacy ack path: ${TMP_HOME}/acks/<sha256(WORK_DIR).slice(0,16)>.claude.json
...
//
// T10 note: The test-only env var CODEX_DELEGATION_PERMISSION_TIMEOUT_MS overrides the
...
const SCRIPT = join(REPO_ROOT, 'packages', 'plugin-delegate', 'scripts', 'delegate.mjs');
const MOCK_CLAUDE = join(REPO_ROOT, 'tools', 'mock-claude');
const MOCK_CODEX = join(REPO_ROOT, 'tools', 'mock-codex');
const FORMAT_LIB = join(REPO_ROOT, 'packages', 'plugin-delegate', 'scripts', 'lib', 'format.mjs');
...
beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'dispatcher-delegation-'));
...
function runDispatcher(args, { cwd = WORK_DIR, env: extraEnv = {} } = {}) {
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...process.env,
      ...extraEnv,
      CODEX_DELEGATION_HOME: TMP_HOME,
      CODEX_DELEGATION_MOCK_CLAUDE_HOME: MOCK_HOME,
      ...
      CODEX_DELEGATION_REVIEW_RECONCILE_DELAY_MS: '0',
 ...
 */
function ackPath() {
  return join(TMP_HOME, 'acks', `${ackHex(WORK_DIR)}.claude.json`);
}
...
describe('setup --json', () => {
  ...
  it('fails setup before delegation when Claude Code subscription model access is disabled', () => {
    ...
    const result = runDispatcher(['setup', '--json'], {
      env: { CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath },
    });
...
describe('delegate without --yes (non-TTY stdin)', () => {
  ...
  it('JSON output includes structured ackRequired retry metadata', () => {
    ...
    assert.match(
      ...
      /node ".+packages\/plugin-delegate\/scripts\/delegate\.mjs" "delegate" "--yes" "--json" "--" "hello"/,
    );
...
describe('status --job and --compact ergonomics (Plan 0022 friction polish)', () => {
  it('status --job <id> --json returns one compact redacted job shape', () => {
    ...
    assert.equal(
      parsed.meta.dispatcherPath.endsWith('/packages/plugin-delegate/scripts/delegate.mjs'),
      true,
    );
    assert.ok(
      parsed.job.exactActionHints.result.includes(`/packages/plugin-delegate/scripts/delegate.mjs`),
  ...
  it('exits 0 and prints usage information', () => {
    ...
    assert.ok(
      result.stdout.toLowerCase().includes('usage') ||
        result.stdout.toLowerCase().includes('delegate') ||
...
function writeAck(workspaceRoot) {
  ...
  writeFileSync(
    join(acksDir, `${hex}.claude.json`),
    JSON.stringify(
      { workspaceRoot, provider: 'claude', ackedAt: new Date().toISOString() },
      null,
      2,
    ),
  );
...
describe('followup ack checks target workspace (T8)', () => {
  ...
  it('T8-13: --yes records ack for target workspace (not just caller workspace)', () => {
    ...
    try {
      ...
      const expectedAckPath = join(resolvedTmpHome, 'acks', `${ackHex(workspaceA)}.claude.json`);
      const fallbackAckPath = join(TMP_HOME, 'acks', `${ackHex(workspaceA)}.claude.json`);
      assert.ok(
...
describe('followup target-workspace ack scoping (plan 0002 T12)', () => {
  ...
  it('T12-1: caller workspace ack does not satisfy target workspace ack requirement', () => {
    ...
    try {
      ...
      const targetAckFile = join(TMP_HOME, 'acks', `${ackHex(workspaceA)}.claude.json`);
      const targetAckFileResolved = join(
        realpathSync(TMP_HOME),
        'acks',
        `${ackHex(workspaceA)}.claude.json`,
      );
      assert.ok(
  ...
  it('T12-2: --all --yes from caller targeting different workspace exits 0 and records ack for target only', () => {
    ...
    try {
      ...
      const callerAckFile = join(TMP_HOME, 'acks', `${ackHex(WORK_DIR)}.claude.json`);
      const callerAckFileResolved = join(
        resolvedTmpHome,
        'acks',
        `${ackHex(WORK_DIR)}.claude.json`,
      );
      assert.ok(
  ...
  it('T12-9: followup without --all on same-workspace job requires ack and --yes records it', () => {
    ...
    const resolvedTmpHome = realpathSync(TMP_HOME);
    const ackFile = join(TMP_HOME, 'acks', `${ackHex(WORK_DIR)}.claude.json`);
    const ackFileResolved = join(resolvedTmpHome, 'acks', `${ackHex(WORK_DIR)}.claude.json`);
    assert.ok(
...
describe('isolation: real ~/.claude and ~/.codex are not touched', () => {
  it('test job IDs do not appear under the real ~/.codex/codex-delegation-plugin/jobs/', () => {
    ...
    // Job should NOT be in the real ~/.codex/codex-delegation-plugin/jobs/
    const realJobsDir = join(
    ...
    assert.ok(
      ...
      `job record must NOT appear in real ~/.codex/codex-delegation-plugin/jobs/: ${realJobPath}`,
    );
  });

  it('TMP_HOME is isolated from the real ~/.codex directory', () => {
    const realCodex = join(process.env.HOME ?? '/root', '.codex', 'codex-delegation-plugin');
    assert.notEqual(TMP_HOME, realCodex, 'TMP_HOME must not equal the real delegation home');
...
//     via CODEX_DELEGATION_PERMISSION_TIMEOUT_MS).
...
//   the dispatcher subprocess can pick it up via CODEX_DELEGATION_MOCK_CLAUDE_CONFIG.
// ==========================================================================
...
describe('followup permission handoff (plan 0002 T10)', () => {
  ...
  it('T10-1: non-TTY stdin triggers null callback, exits 1 with "claude attach" hint', () => {
    ...
    const result = spawnSync(
      ...
      {
        cwd: WORK_DIR,
        env: { ...t10Env({ CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath }) },
        encoding: 'utf8',
      },
    );
  ...
  it(
    ...
    async () => {
      ...
      const cfgPath = writeMockClaudeConfig(MOCK_HOME, { permissionStall: true });
      const env = t10Env({ CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath });
  ...
  it(
    ...
    async () => {
      ...
      const cfgPath = writeMockClaudeConfig(MOCK_HOME, { permissionStall: true });
      // CODEX_DELEGATION_PERMISSION_TIMEOUT_MS=100 makes the readline timeout fire in 100ms.
      const env = t10Env({
        CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath,
        CODEX_DELEGATION_PERMISSION_TIMEOUT_MS: '100',
      });
...
describe('review --json runtime errors (Plan 0025)', () => {
  it('prints parseable JSON to stdout when same-session send fails at runtime', () => {
    ...
    const result = runDispatcher(['review', jobId, '--json', '--yes'], {
      env: {
        CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath,
        CODEX_DELEGATION_PROMPT_REGISTER_TIMEOUT_MS: '1',
      },
    });
...
describe('review --yes records ack for target workspace (T4)', () => {
  it('T4-16: --yes records an ack for the target workspace and proceeds', () => {
    ...
    try {
      ...
      const resolvedTmpHome = realpathSync(TMP_HOME);
      const targetAckFile = join(TMP_HOME, 'acks', `${ackHex(targetWorkspace)}.claude.json`);
      const targetAckFileResolved = join(
        resolvedTmpHome,
        'acks',
        `${ackHex(targetWorkspace)}.claude.json`,
      );
      assert.ok(
        existsSync(targetAckFile) || existsSync(targetAckFileResolved),
        `expected ack file for target workspace at ${targetAckFile}`,
...
// exits. For timeout tests we use CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS=1
...
describe('adversarial-review session name (T6)', () => {
  it('T6-2: new review session name starts with codex:<repo-basename>:review-<targetJobId.slice(0,12)>', () => {
    ...
    const repoBasename = 'codex-delegation-plugin';
  ...
  it('Plan 0024: --fail-on medium exits non-zero after emitting review JSON', () => {
    ...
    const result = runDispatcher(
      ['adversarial-review', jobId, '--yes', '--json', '--fail-on', 'medium'],
      {
        env: {
          CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath,
          CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS: '1',
        },
      },
    );
...
describe('adversarial-review timeout fires when review never completes (T6)', () => {
  ...
  it('T6-19: times out with non-zero exit, exact timeout message, review job status failed, review.failed event written', () => {
    ...
    const result = runDispatcher(['adversarial-review', jobId, '--yes'], {
      env: {
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS: '1',
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS: '50',
        CODEX_DELEGATION_MOCK_CLAUDE_CONFIG: cfgPath,
      },
    });
...
describe('adversarial-review env timeout parse (T6)', () => {
  it('T6-20a: valid small timeout (300ms) is accepted; error message uses rounded minutes (0)', () => {
    ...
    const result = runDispatcher(['adversarial-review', jobId, '--yes'], {
      env: {
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS: '300',
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS: '50',
      },
    });
  ...
  it('T6-20b: NaN env value falls back to 30-min default (review completes successfully)', () => {
    ...
    const result = runDispatcher(['adversarial-review', jobId, '--yes'], {
      env: {
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS: 'abc',
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS: '50',
      },
    });
  ...
  it('T6-20c: negative env value falls back to 30-min default (review completes successfully)', () => {
    ...
    const result = runDispatcher(['adversarial-review', jobId, '--yes'], {
      env: {
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_TIMEOUT_MS: '-100',
        CODEX_DELEGATION_ADVERSARIAL_REVIEW_POLL_MS: '50',
      },
    });
...
describe('delegate/followup command-specific help', () => {
  it('delegate --help prints focused delegate usage and unattended-run guidance', () => {
    ...
    assert.ok(result.stdout.startsWith('Usage: delegate delegate'), result.stdout);
  ...
  it('followup --help prints focused followup usage and resultPending guidance', () => {
    ...
    assert.ok(result.stdout.startsWith('Usage: delegate followup'), result.stdout);
...
describe('workflow --help command-specific output', () => {
  it('mentions attach and that --yes does not approve the workflow gate', () => {
    ...
    assert.ok(result.stdout.startsWith('Usage: delegate workflow'), result.stdout);
...
describe('upgrade command', () => {
  ...
  it('upgrade --dry-run --json prints the reinstall plan without invoking codex plugin commands', () => {
    ...
    const result = runDispatcher(['upgrade', '--dry-run', '--json'], {
      env: { CODEX_DELEGATION_MOCK_CODEX_CONFIG: cfgPath },
    });
    ...
    assert.equal(parsed.target.plugin, 'delegate@codex-delegation-plugin');
  ...
  it('upgrade --local --dry-run --json targets the local marketplace install', () => {
    ...
    assert.equal(parsed.target.marketplace, 'codex-delegation-plugin-local');
    assert.equal(parsed.target.plugin, 'delegate@codex-delegation-plugin-local');
    assert.deepEqual(
      parsed.commands.map((step) => step.args),
      [
        ['plugin', 'remove', 'delegate@codex-delegation-plugin-local'],
        ['plugin', 'add', 'delegate@codex-delegation-plugin-local', '--json'],
        ['plugin', 'list'],
      ],
    );
  });
  ...
  it('upgrade --public --dry-run --json targets the public Git marketplace install', () => {
    ...
    assert.equal(parsed.target.marketplace, 'codex-delegation-plugin');
    assert.equal(parsed.target.plugin, 'delegate@codex-delegation-plugin');
  ...
  it('upgrade without --yes defaults to a dry-run plan', () => {
    ...
    assert.equal(result.status, 0, `expected exit 0; stderr: ${result.stderr}`);
    assert.ok(result.stdout.includes('Codex Delegation upgrade plan'), result.stdout);
  ...
  it('upgrade --yes --json refreshes the marketplace and reinstalls the plugin through codex', () => {
    ...
    assert.deepEqual(readCommandLog(logPath), [
      ['plugin', 'marketplace', 'upgrade', 'codex-delegation-plugin'],
      ['plugin', 'remove', 'delegate@codex-delegation-plugin'],
      ['plugin', 'add', 'delegate@codex-delegation-plugin', '--json'],
      ['plugin', 'list'],
    ]);
    assert.ok(
  ...
  it('upgrade --yes prints human output after refreshing the stable cache symlink', () => {
    ...
    assert.equal(result.status, 0, `expected exit 0; stderr: ${result.stderr}`);
    assert.ok(result.stdout.includes('Codex Delegation plugin refresh complete.'), result.stdout);
  ...
  it('upgrade falls back to marketplace add if marketplace upgrade is not registered', () => {
    ...
    const cfgPath = writeMockCodexConfig({
      commandLogPath: logPath,
      pluginFailures: {
        'plugin marketplace upgrade codex-delegation-plugin': 'not registered',
      },
    });
    ...
    assert.deepEqual(readCommandLog(logPath), [
      ['plugin', 'marketplace', 'upgrade', 'codex-delegation-plugin'],
      ['plugin', 'marketplace', 'add', 'https://github.com/wu-hongjun/codex-delegation-plugin'],
      ['plugin', 'remove', 'delegate@codex-delegation-plugin'],
      ['plugin', 'add', 'delegate@codex-delegation-plugin', '--json'],
      ['plugin', 'list'],
    ]);
  ...
  it('upgrade --help prints focused upgrade usage', () => {
    ...
    assert.ok(result.stdout.startsWith('Usage: delegate upgrade'), result.stdout);
...
describe('deep-research --help command-specific output', () => {
  it('mentions WebSearch, attach, and possible workflow approval gates', () => {
    ...
    assert.ok(result.stdout.startsWith('Usage: delegate deep-research'), result.stdout);
...
describe('workflows --help command-specific output', () => {
  it('mentions read-only inspection and deep-research inclusion', () => {
    ...
    assert.ok(result.stdout.startsWith('Usage: delegate workflows'), result.stdout);

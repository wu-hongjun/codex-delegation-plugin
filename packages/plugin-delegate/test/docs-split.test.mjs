...
//
//   1. marketplace/plugins/delegate/README.md
...
//
//   2. packages/plugin-delegate/README.md
...
const PLUGIN_README = resolve(REPO_ROOT, 'packages', 'plugin-delegate', 'README.md');
...
describe('docs split — three distinct surfaces (Plan 0006 T12)', () => {
  ...
  describe('marketplace README — end-user surface', () => {
    ...
    it('contains exact install commands', () => {
      ...
      assert.ok(
        content.includes('codex plugin add "delegate@codex-delegation-plugin"'),
        'marketplace README must contain verbatim `codex plugin add "delegate@codex-delegation-plugin"`',
      );
      assert.ok(
      ...
      assert.ok(
        content.includes('codex plugin add "delegate@codex-delegation-plugin-local"'),
        'marketplace README must contain verbatim `codex plugin add "delegate@codex-delegation-plugin-local"`',
      );
    });
  ...
  describe('plugin README — comprehensive developer surface', () => {
    ...
    it('`## Install locally` references the committed `marketplace/` install flow', () => {
      ...
      assert.match(
        ...
        'plugin README install section must reference the `delegate@codex-delegation-plugin-local` plugin add command',
      );
    });
    ...
    it('`## Install locally` points at marketplace README and RELEASING.md', () => {
      ...
      assert.match(
        body,
        /marketplace\/plugins\/delegate\/README\.md/,
  ...
  describe('root README — workspace overview', () => {
    ...
    it('contains a Quick start section pointing at all four canonical surfaces', () => {
      ...
      assert.match(
        content,
        /packages\/plugin-delegate\/README\.md/,
...
describe('docs polish — Stage 4 audit findings (Plan 0006 Stage 4)', () => {
  ...
  describe('F-2: root README status must describe the maintained release surface', () => {
    it('root README must state the current skill and provider surface', () => {
      const content = readFileSync(ROOT_README, 'utf8');
      assert.match(content, /currently ships 24 Codex skills/i);
      assert.match(content, /Claude Code background-session delegation/i);
      assert.match(content, /Antigravity print-mode delegation/i);
    });

    it('root README must point to the manifest, release checklist, and historical plan records', () => {
      const content = readFileSync(ROOT_README, 'utf8');
      assert.match(content, /packages\/plugin-delegate\/\.codex-plugin\/plugin\.json/);
      assert.match(content, /documentation\/RELEASING\.md/);
      assert.match(content, /plan history/i);
      assert.match(content, /reports are records rather than current setup docs/i);
    });
  });
});
...
describe('$claude-workflow documentation — plugin README (Plan 0008 T5)', () => {
  ...
  it('plugin README Direct dispatcher usage section mentions the workflow subcommand', () => {
    const content = readFileSync(PLUGIN_README, 'utf8');
    assert.ok(
      content.includes('delegate.mjs workflow'),
      'plugin README must document "delegate.mjs workflow" in the Direct dispatcher usage section',
    );
  });
});

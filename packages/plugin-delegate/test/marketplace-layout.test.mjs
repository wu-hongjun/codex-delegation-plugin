...
// packages/plugin-delegate/test/ -> up 3 levels to repo root
const REPO_ROOT = resolve(here, '..', '..', '..', '..');
const MARKETPLACE_ROOT = resolve(REPO_ROOT, 'marketplace');
const SOURCE_PLUGIN_ROOT = resolve(REPO_ROOT, 'packages', 'plugin-delegate');
...
const MARKETPLACE_PLUGIN_ROOT = resolve(MARKETPLACE_ROOT, 'plugins', 'delegate');
...
const MARKETPLACE_ENTRY_SCRIPT = resolve(MARKETPLACE_SCRIPTS_DIR, 'delegate.mjs');
...
describe('marketplace/ layout (Plan 0006 T2)', () => {
  it('root .agents/plugins/marketplace.json exposes the public Git marketplace shape', () => {
    ...
    assert.equal(
      ...
      `root marketplace.json name must be "codex-delegation-plugin"; got "${parsed.name}"`,
    );
    assert.equal(
      parsed.interface?.displayName,
      'Codex Delegation',
      `root marketplace.json interface.displayName must be "Codex Delegation"; got "${parsed.interface?.displayName}"`,
    );
    ...
    assert.equal(
      ...
      'delegate',
      `root marketplace.json plugins[0].name must be "delegate"; got "${parsed.plugins[0].name}"`,
    );
    assert.equal(
    ...
    assert.equal(
      ...
      `root marketplace.json plugins[0].source.path must be "./marketplace/plugins/delegate"; got "${parsed.plugins[0].source?.path}"`,
    );
  });
  ...
  it('marketplace/.agents/plugins/marketplace.json exists and has required shape', () => {
    ...
    assert.equal(
      ...
      `marketplace.json name must be "codex-delegation-plugin-local"; got "${parsed.name}"`,
    );
    assert.equal(
      parsed.interface?.displayName,
      'Codex Delegation Local',
      `marketplace.json interface.displayName must be "Codex Delegation Local"; got "${parsed.interface?.displayName}"`,
    );
    ...
    assert.equal(
      parsed.plugins[0].name,
      'delegate',
      `marketplace.json plugins[0].name must be "delegate"; got "${parsed.plugins[0].name}"`,
    );
    assert.equal(
      parsed.plugins[0].source?.source,
      'local',
    ...
    assert.equal(
      parsed.plugins[0].source?.path,
      './plugins/delegate',
      `marketplace.json plugins[0].source.path must be "./plugins/delegate"; got "${parsed.plugins[0].source?.path}"`,
    );
  });
  ...
  it('marketplace/plugins/delegate/.codex-plugin/plugin.json exists', () => {
    assert.ok(
  ...
  it('marketplace plugin.json is byte-identical to packages/plugin-delegate/.codex-plugin/plugin.json', () => {
    assert.ok(
  ...
  // ========================================================================
  // Check 6: marketplace delegate.mjs exists and has executable bit
  // ========================================================================

  it('marketplace/plugins/delegate/scripts/delegate.mjs exists and has the user-executable bit set', () => {
    assert.ok(
      existsSync(MARKETPLACE_ENTRY_SCRIPT),
      `delegate.mjs not found at ${MARKETPLACE_ENTRY_SCRIPT}`,
    );
    const stat = statSync(MARKETPLACE_ENTRY_SCRIPT);
    assert.ok(
      (stat.mode & 0o100) !== 0,
      `delegate.mjs must have the user-executable bit set; mode = ${stat.mode.toString(8)}`,
    );
  });
  ...
  // ========================================================================
  // Check 8: no forbidden internal/test files under marketplace/plugins/delegate/
  //
  ...
  it('no forbidden internal/test files appear under marketplace/plugins/delegate/', () => {
    assert.ok(
  ...
  it('marketplace/plugins/delegate/README.md contains no OQ4 forbidden cost-claim tokens', () => {
    assert.ok(
...
describe('marketplace packaging procedure (Plan 0006 T4)', () => {
  ...
  it('MANIFEST.md lists every derived file under marketplace/plugins/delegate/', () => {
    assert.ok(existsSync(MANIFEST_MD), `MANIFEST.md not found at ${MANIFEST_MD}`);
    assert.ok(
      existsSync(MARKETPLACE_PLUGIN_ROOT),
      `marketplace plugin root not found at ${MARKETPLACE_PLUGIN_ROOT}`,
  ...
  // ========================================================================
  // T4-3: No file under marketplace/plugins/delegate/ is outside
  //        the MANIFEST.md list (reverse direction)
  // ========================================================================

  it('marketplace/plugins/delegate/ contains no file outside the MANIFEST.md list (reverse check)', () => {
    assert.ok(existsSync(MANIFEST_MD), `MANIFEST.md not found at ${MANIFEST_MD}`);
    assert.ok(
      existsSync(MARKETPLACE_PLUGIN_ROOT),
      `marketplace plugin root not found at ${MARKETPLACE_PLUGIN_ROOT}`,
  ...
  it('all derived files are byte-identical between source and marketplace', () => {
    for (const rel of DERIVED_FILES_ALLOWLIST) {
      ...
      assert.ok(existsSync(srcPath), `source file missing: packages/plugin-delegate/${rel}`);
      assert.ok(
        existsSync(dstPath),
        `marketplace file missing: marketplace/plugins/delegate/${rel}`,
      );
  ...
  // ========================================================================
  // T4-7: scripts/delegate.mjs in marketplace has executable bit
  //        (already covered by T2 Check 6 — verify it still passes)
  // ========================================================================

  it('marketplace/plugins/delegate/scripts/delegate.mjs still has user-executable bit (T2 guard)', () => {
    assert.ok(
      existsSync(MARKETPLACE_ENTRY_SCRIPT),
      `delegate.mjs not found at ${MARKETPLACE_ENTRY_SCRIPT}`,
    );
  ...
  // ========================================================================
  // T4-8: No extra files under marketplace/plugins/delegate/
  //        (T2 Check 8 guard — MANIFEST.md lives at marketplace/MANIFEST.md,
  //         not inside plugins/delegate/, so the walker is unaffected)
  // ========================================================================

  it('no extra files appear under marketplace/plugins/delegate/ beyond the allowlist + README.md', () => {
    assert.ok(
      existsSync(MARKETPLACE_PLUGIN_ROOT),
      `marketplace plugin root not found at ${MARKETPLACE_PLUGIN_ROOT}`,
    );
    ...
    assert.deepEqual(
      extras,
      [],
      `unexpected files found under marketplace/plugins/delegate/: ${JSON.stringify(extras)}`,
    );
  });
...
// tree under marketplace/plugins/delegate/. They are purely static:
...
describe('marketplace bundled-dependency tree (Plan 0006 T9.5)', () => {
  ...
  it('marketplace/plugins/delegate/node_modules/ directory exists', () => {
  ...
  it('bundled @codex-delegation/runtime/package.json has required shape', () => {
    ...
    assert.equal(pkg.name, '@codex-delegation/runtime', 'runtime package.json name');
  ...
  it('bundled @codex-delegation/driver-claude-code/package.json has required shape and pinned deps', () => {
    const pkgPath = resolve(BUNDLED_DRIVER, 'package.json');
    assert.ok(existsSync(pkgPath), `driver package.json not found at ${pkgPath}`);
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf8'));
    assert.equal(pkg.name, '@codex-delegation/driver-claude-code', 'driver package.json name');
    ...
    assert.equal(
      ...
      `driver must pin @codex-delegation/runtime to ${BUNDLED_VERSION_MARKER}`,
    );
  });
  ...
  it('bundled @codex-delegation/driver-agy-cli package has the required shape', () => {
    ...
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf8'));
    assert.equal(pkg.name, '@codex-delegation/driver-agy-cli', 'agy driver package.json name');
    ...
    assert.equal(
      pkg?.dependencies?.['@codex-delegation/runtime'],
      BUNDLED_VERSION_MARKER,
      `agy driver must pin @codex-delegation/runtime to ${BUNDLED_VERSION_MARKER}`,
    );
    assert.equal(
      pkg?.dependencies?.['node-pty'],
      '1.2.0-beta.13',

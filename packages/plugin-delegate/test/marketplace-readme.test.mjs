...
// documentation in marketplace/plugins/delegate/README.md and
...
const MARKETPLACE_README = resolve(REPO_ROOT, 'marketplace', 'plugins', 'delegate', 'README.md');
...
const CMD_PLUGIN_ADD = 'codex plugin add "delegate@codex-delegation-plugin-local"';
const CMD_GIT_MARKETPLACE_ADD =
  'codex plugin marketplace add https://github.com/wu-hongjun/codex-delegation-plugin';
const CMD_GIT_PLUGIN_ADD = 'codex plugin add "delegate@codex-delegation-plugin"';
...
const CMD_PLUGIN_REMOVE = 'codex plugin remove "delegate@codex-delegation-plugin-local"';
const CMD_MARKETPLACE_REMOVE = 'codex plugin marketplace remove "codex-delegation-plugin-local"';
const CMD_GIT_PLUGIN_REMOVE = 'codex plugin remove "delegate@codex-delegation-plugin"';
const CMD_GIT_MARKETPLACE_REMOVE = 'codex plugin marketplace remove "codex-delegation-plugin"';
...
const GIT_MARKETPLACE_NAME = 'codex-delegation-plugin';
...
describe('marketplace install procedure docs (Plan 0006 T6)', () => {
  ...
  it('marketplace/plugins/delegate/README.md exists and is non-empty', () => {
  ...
  it('README contains the verbatim `codex plugin add "delegate@codex-delegation-plugin-local"` command', () => {
    const content = readFileSync(MARKETPLACE_README, 'utf8');
    assert.ok(
  ...
  // ========================================================================
  // T6-12 (optional): README contains the marketplace name `codex-delegation-plugin-local`
  // ========================================================================

  it('README contains the marketplace name `codex-delegation-plugin-local`', () => {
    const content = readFileSync(MARKETPLACE_README, 'utf8');
    assert.ok(
...
describe('marketplace upgrade procedure docs (Plan 0006 T7)', () => {
  ...
  it('README contains the Git marketplace upgrade flow', () => {
    const content = readFileSync(MARKETPLACE_README, 'utf8');
    assert.ok(
      content.includes('codex plugin marketplace upgrade "codex-delegation-plugin"'),
...
// (Git checkout and plugin job records under the delegation home).
...
describe('marketplace uninstall procedure docs (Plan 0006 T8)', () => {
  ...
  // ========================================================================
  // T8-8: README states uninstall does not delete delegation-home job records
  // ========================================================================

  it('README states uninstall does not delete plugin job records / delegation-home data', () => {
    const content = readFileSync(MARKETPLACE_README, 'utf8');
    assert.match(
      content,
      /(does not|not).*delete.*(job records|delegation home|delegation-home)/i,
      'README.md must state that uninstall does not delete delegation-home job records',
    );
  });
...
describe('marketplace README documents macOS-safe dispatcher and wait timeout recovery', () => {
  it('warns that bare delegate is Apple clang and points at exact dispatcher hints', () => {
    const content = readFileSync(MARKETPLACE_README, 'utf8');
    assert.ok(
      content.includes('/usr/bin/delegate') && content.includes('Apple clang'),
      'marketplace README must explain the macOS delegate command collision',
    );
    assert.ok(
  ...
  it('documents timeoutRecovery and the stable current dispatcher path', () => {
    ...
    assert.ok(
      content.includes('delegate/current/scripts/delegate.mjs'),

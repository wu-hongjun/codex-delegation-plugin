...
describe('consolidated release checklist (Plan 0006 T11)', () => {
  ...
  // ========================================================================
  // T11-8: Tagging section requires smoke + CI before an annotated tag
  // ========================================================================

  it('Tagging section gates on smoke + CI and creates an annotated tag', () => {
    const content = readFileSync(RELEASING_MD, 'utf8');
    assert.match(
      content,
      /git tag -a v0\.[xX]\.[yY] -m/,
      'Tagging section must include the annotated `git tag -a v0.x.y -m ...` command pattern',
    );

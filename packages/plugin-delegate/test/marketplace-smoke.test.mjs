...
//   - marketplace/plugins/delegate/README.md smoke-test pointer
...
describe('release-smoke procedure (Plan 0006 T9)', () => {
  ...
  // ========================================================================
  // T9-8: script includes `codex plugin add delegate@codex-delegation-plugin-local`
  // ========================================================================
  ...
  it('smoke script derives expected plugin version from the marketplace plugin.json (no hard-coded literal)', () => {
    ...
    assert.match(
      body,
      /plugins\/delegate\/\.codex-plugin\/plugin\.json/,
  ...
  it('smoke surfaces contain no OQ4 forbidden tokens and no Plan 0004 benchmark vocabulary', () => {
    const surfaces = {
      ...
      'marketplace/plugins/delegate/README.md': readFileSync(MARKETPLACE_README, 'utf8'),
    };
...
describe('smoke helper dispatcher-execution check (Plan 0006 T9.5)', () => {
  ...
  it('smoke script source contains a STEP 5.5 dispatcher-execution step', () => {
    ...
    // Must spawn the delegate.mjs setup command
    assert.ok(
      body.includes('delegate.mjs') && body.includes('setup'),
      'smoke script STEP 5.5 must spawn delegate.mjs setup',
    );
  });
...
//   1. `packages/plugin-delegate/.codex-plugin/plugin.json` is the SOURCE OF
//      TRUTH for the shipped plugin version.
//   2. `marketplace/plugins/delegate/.codex-plugin/plugin.json` is a
...
const PLUGIN_PACKAGE_JSON = resolve(REPO_ROOT, 'packages', 'plugin-delegate', 'package.json');
...
const WORKSPACE_PACKAGE_JSON_FILES = [
  ...
  { label: 'packages/plugin-delegate', path: PLUGIN_PACKAGE_JSON },
...
describe('plugin versioning scheme (Plan 0006 T10)', () => {
  ...
  it('RELEASING.md Version Bump section names source of truth + key commands', () => {
    const content = readFileSync(RELEASING_MD, 'utf8');
    assert.ok(
      content.includes('packages/plugin-delegate/.codex-plugin/plugin.json'),
      'RELEASING.md must name `packages/plugin-delegate/.codex-plugin/plugin.json` as the source of truth',
    );
    assert.match(
      content,

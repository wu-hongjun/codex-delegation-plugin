...
// PLUGIN_ROOT = packages/plugin-delegate/
...
describe('plugin.json.name', () => {
  it('equals "delegate"', () => {
    ...
    assert.equal(manifest.name, 'delegate', `expected name "delegate", got "${manifest.name}"`);
  });
});
...
describe('plugin.json.version', () => {
  ...
  it('keeps the 0.6.1 release base with optional local cachebuster metadata', () => {
    const manifest = readManifest();
    assert.match(
      manifest.version,
      /^0\.6\.1(?:\+codex\.(?:[0-9]+|local-[0-9]{14}))?$/,
      `expected 0.6.1 release base, got "${manifest.version}"`,
    );
  });
});
...
describe('each SKILL.md references scripts/delegate.mjs', () => {
  for (const name of SKILL_NAMES) {
    it(`${name}: body contains "scripts/delegate.mjs"`, () => {
      const body = readFileSync(skillPath(name), 'utf8');
      assert.ok(
        body.includes('scripts/delegate.mjs'),
        `${name}: SKILL.md does not reference "scripts/delegate.mjs"`,
      );
    });
  }
});
...
describe('claude-delegate/SKILL.md does not auto-inject --yes on dispatcher run lines', () => {
  it('no line containing both "delegate.mjs" and "delegate" also contains " --yes"', () => {
    const body = readFileSync(skillPath('claude-delegate'), 'utf8');
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('delegate'))
...
describe('claude-followup/SKILL.md does not auto-inject --yes on dispatcher run lines', () => {
  it('no line containing both "delegate.mjs" and "followup" also contains " --yes"', () => {
    const body = readFileSync(skillPath('claude-followup'), 'utf8');
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('followup'))
      .filter((line) => line.includes(' --yes'));
...
describe('no hooks.json at package root', () => {
  it('hooks.json does not exist in packages/plugin-delegate/', () => {
    assert.equal(
      existsSync(join(PLUGIN_ROOT, 'hooks.json')),
      false,
      'hooks.json must not exist in packages/plugin-delegate/',
    );
  });

  it('hooks/ directory does not exist in packages/plugin-delegate/', () => {
    assert.equal(
      existsSync(join(PLUGIN_ROOT, 'hooks')),
      false,
      'hooks/ directory must not exist in packages/plugin-delegate/',
    );
  });
});
...
describe('no .codex-marketplace config file exists', () => {
  it('.codex-marketplace.json does not exist in packages/plugin-delegate/', () => {
    assert.equal(
      existsSync(join(PLUGIN_ROOT, '.codex-marketplace.json')),
      false,
      '.codex-marketplace.json must not exist in packages/plugin-delegate/',
    );
  });
  ...
  it('.codex-marketplace does not exist (directory variant)', () => {
    assert.equal(
      ...
      false,
      '.codex-marketplace must not exist in packages/plugin-delegate/',
    );
  });
});
...
describe('claude-review/SKILL.md: --yes auto-injection guard', () => {
  it('no dispatcher run line for "review" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes(' review'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-adversarial-review/SKILL.md: --yes auto-injection guard', () => {
  it('no dispatcher run line for "adversarial-review" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('adversarial-review'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-workflow/SKILL.md: run line invokes dispatcher with "workflow" subcommand', () => {
  it('body contains "delegate.mjs" and "workflow"', () => {
    const body = readFileSync(skillPath('claude-workflow'), 'utf8');
    assert.ok(
      body.includes('delegate.mjs'),
      'claude-workflow/SKILL.md must reference scripts/delegate.mjs',
    );
    assert.ok(
...
describe('claude-workflow/SKILL.md: --yes auto-injection guard', () => {
  it('no dispatcher run line for "workflow" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('workflow'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-workflow/SKILL.md: --allow-edit is not referenced in run lines', () => {
  it('no dispatcher run line contains "--allow-edit"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs'))
...
describe('claude-goal/SKILL.md: run line invokes dispatcher with "goal" subcommand (T3b Plan 0010)', () => {
  it('body contains "delegate.mjs" and "goal"', () => {
    const body = readFileSync(skillPath('claude-goal'), 'utf8');
    assert.ok(
      body.includes('delegate.mjs'),
      'claude-goal/SKILL.md must reference scripts/delegate.mjs',
    );
...
describe('claude-goal/SKILL.md: --yes auto-injection guard (T3b Plan 0010)', () => {
  it('no dispatcher run line for "goal" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('goal'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-fork/SKILL.md: run line invokes dispatcher with "fork" subcommand (Plan 0011)', () => {
  it('body contains "delegate.mjs" and "fork"', () => {
    const body = readFileSync(skillPath('claude-fork'), 'utf8');
    assert.ok(
      body.includes('delegate.mjs'),
      'claude-fork/SKILL.md must reference scripts/delegate.mjs',
    );
...
describe('claude-fork/SKILL.md: --yes auto-injection guard (Plan 0011)', () => {
  it('no dispatcher run line for "fork" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('fork'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-batch/SKILL.md: run line invokes dispatcher with "batch" subcommand (Plan 0011)', () => {
  it('body contains "delegate.mjs" and "batch"', () => {
    const body = readFileSync(skillPath('claude-batch'), 'utf8');
    assert.ok(
      body.includes('delegate.mjs'),
      'claude-batch/SKILL.md must reference scripts/delegate.mjs',
    );
    assert.ok(
      body.includes('batch'),
...
describe('claude-batch/SKILL.md: --yes auto-injection guard (Plan 0011)', () => {
  it('no dispatcher run line for "batch" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('batch'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-deep-research/SKILL.md: run line invokes dispatcher with "deep-research" subcommand (Plan 0013)', () => {
  it('body contains "delegate.mjs" and "deep-research"', () => {
    const body = readFileSync(skillPath('claude-deep-research'), 'utf8');
    assert.ok(
      body.includes('delegate.mjs'),
      'claude-deep-research/SKILL.md must reference scripts/delegate.mjs',
    );
    assert.ok(
...
describe('claude-deep-research/SKILL.md: --yes auto-injection guard (Plan 0013)', () => {
  it('no dispatcher run line for "deep-research" subcommand contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('deep-research'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('claude-workflows/SKILL.md: run line invokes dispatcher with "workflows" subcommand (Plan 0016)', () => {
  it('body contains "delegate.mjs" and "workflows"', () => {
    const body = readFileSync(skillPath('claude-workflows'), 'utf8');
    assert.ok(
      body.includes('delegate.mjs'),
      'claude-workflows/SKILL.md must reference scripts/delegate.mjs',
    );
    assert.ok(
...
describe('claude-workflows/SKILL.md: no --yes auto-injection (Plan 0016)', () => {
  it('no dispatcher run line contains " --yes"', () => {
    ...
    const offendingLines = body
      .split('\n')
      .filter((line) => line.includes('delegate.mjs') && line.includes('workflows'))
      .filter((line) => line.includes(' --yes'));
    assert.equal(
      offendingLines.length,
      0,
...
describe('plugin.json interface.defaultPrompt contains Codex Delegation upgrade entry', () => {
  it('includes "Upgrade or refresh the installed Codex Delegation plugin."', () => {
    const manifest = readManifest();
    const dp = manifest.interface?.defaultPrompt;
    assert.ok(Array.isArray(dp), 'interface.defaultPrompt must be an array');
    assert.ok(
      dp.includes('Upgrade or refresh the installed Codex Delegation plugin.'),
      `interface.defaultPrompt must contain "Upgrade or refresh the installed Codex Delegation plugin."; got: ${JSON.stringify(dp)}`,
    );
  });
});

describe('plugin.json interface.defaultPrompt contains Codex Delegation doctor entry', () => {
...
describe('operator-friction skill docs mention safe dispatcher paths and timeout recovery', () => {
  it('claude-status warns not to rewrite action hints to bare delegate on macOS', () => {
    const body = readFileSync(skillPath('claude-status'), 'utf8');
    assert.ok(
      body.includes('Apple clang') && body.includes('exactActionHints'),
      'claude-status skill must warn about bare delegate on macOS and mention exactActionHints',
    );
  });

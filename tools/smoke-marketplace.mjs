 ...
 * assertions only and prints the thirteen-skill manual checklist for the
 ...
 * script's shape (--help text, 13 skill names, isolation invariants,
...
// All thirteen skills shipped by the cc-plugin-codex marketplace plugin. Order
...
function printHelp() {
  process.stdout.write(
    [
      ...
      'skill-invocation interface, so the thirteen-skill discovery check must',
      ...
      '',
      'Thirteen skills covered by the manual TUI checklist:',
      ...SKILL_NAMES.map((s) => `  - $${s}`),
      '',
    ].join('\n'),
  );
...
process.stdout.write(
  [
    ...
    `CODEX_HOME and verify each of the 13 skills below is recognized.`,
    '',

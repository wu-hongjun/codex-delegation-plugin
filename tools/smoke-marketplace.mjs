 ...
 * assertions only and prints the sixteen-skill manual checklist for the
 ...
 *   8. Prints the sixteen-skill manual TUI checklist.
 ...
 * script's shape (--help text, 16 skill names, isolation invariants,
...
// All sixteen skills shipped by the cc-plugin-codex marketplace plugin. Order
...
function printHelp() {
  process.stdout.write(
    [
      ...
      'skill-invocation interface, so the sixteen-skill discovery check must',
      ...
      '',
      'Sixteen skills covered by the manual TUI checklist:',
      ...SKILL_NAMES.map((s) => `  - $${s}`),
      '',
    ].join('\n'),
  );
...
process.stdout.write(
  [
    ...
    `CODEX_HOME and verify each of the 16 skills below is recognized.`,
    '',
    ...
    '  - The other 15 skills do not return "unknown skill" or',

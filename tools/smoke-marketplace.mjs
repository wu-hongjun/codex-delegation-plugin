 ...
 * assertions only and prints the nine-skill manual checklist for the
 ...
 *   8. Prints the nine-skill manual TUI checklist.
 ...
 * script's shape (--help text, 9 skill names, isolation invariants,
...
// All nine skills shipped by the cc-plugin-codex marketplace plugin. Order
...
function printHelp() {
  process.stdout.write(
    [
      ...
      'skill-invocation interface, so the nine-skill discovery check must',
      ...
      '',
      'Nine skills covered by the manual TUI checklist:',
      ...SKILL_NAMES.map((s) => `  - $${s}`),
      '',
    ].join('\n'),
  );
...
process.stdout.write(
  [
    ...
    `CODEX_HOME and verify each of the 9 skills below is recognized.`,
    '',
    ...
    '  - The other 7 skills do not return "unknown skill" or',
